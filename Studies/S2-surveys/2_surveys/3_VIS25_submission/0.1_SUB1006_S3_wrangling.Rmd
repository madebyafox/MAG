...
ref_sd_questions <-  c("DESIGN","DATA","POLITICS","ALIGN","INTENT","TRUST","BEAUTY")
ref_sd_questions_z <-  c("DESIGN_z","DATA_z","POLITICS_z", "ALIGN_z","INTENT_z","TRUST_z","BEAUTY_z")
...
ref_sd_questions_abs <-ref_sd_questions
...
right <- c("professional/layperson",
...
## DF_SD_QUESTIONS_ABS
df_sd_questions_abs <- df_questions %>% 
  filter(QUESTION %in% ref_sd_questions) %>% 
  mutate(
    PRE = abs(as.numeric(PRE)-50),
    POST = abs(as.numeric(POST)-50)
  ) %>% 
  droplevels()
write.csv(df_sd_questions_abs, file = "data/output/Study_3/df_sd_questions_abs_S3.csv", na="")
saveRDS(df_sd_questions_abs, file = "data/output/Study_3/df_sd_questions_abs_S3.csv.rds")
