...
#### RE-RUN THIS PARTIAL R2 ANALYSIS
...
result <- partR2(
  ...
  partbatch = list(
    ...
    "ALIGNN:INTENT" = "INTENT_z:ALIGN_z"
  ),
  nboot = 1000
)
...
saveRDS(result, file = "models/mm.BxIxAD_partR2_batched.rds")
...
Here we see whether the model is also significant in Study 4 (pretest
only)
...
predict trust is also a good fit to the data from Study 4 (pre-tests)
...
################## PLOT MODEL
## PLOT MODEL COEFFICIENTS
plot_model(mm.BxIxAD, type = "est", show.intercept = TRUE, show.p=TRUE, show.values = TRUE) + labs(title = "Model Coefficients") + theme_minimal()
...
No, the model does not constintute a good fit for the Study 4 (pretest) data (only block 1). In this model, INTENT and DATA are no longer significant main effects, and neither interaction is significant either.  
...
# ## (1) BEAUTY X INTENT
# plot_model(best, type = "pred", terms = c("BEAUTY_z","INTENT_z")) + theme_minimal()
# # (2) ALIGN X INTENT
...
################## PLOT MODEL
p + plot_annotation(title = f.BxIxAD, caption="INTENT [inform <--> persuade] \n ALIGN[does NOT share <--> DOES share my values]")
