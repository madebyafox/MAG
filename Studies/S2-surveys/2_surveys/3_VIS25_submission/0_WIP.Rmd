...
df_sd_questions_long_z <- readRDS("data/output/Study_1_2/df_sd_questions_long_z.rds") # only sd questions LONG
...
df_sd_questions_long_all_z <- readRDS("data/output/COMBINED/df_sd_questions_long_ALL_z.rds") # only sd questions LONG


table(df_participants$Study, df_participants$Assigned.Block)
...
## REPORTED IN 4.1.3 
CAN REPORT REPORTED what is sig for general 
...
## MODEL SHIFT(RAW) by question and stimulus
  ...
  m2 <- lmer(value ~ QUESTION * STIMULUS + (1|PID), data = df)
  summary(m2)
  anova(m2)
  
  ## THIS IS THE REPORTED MODEL
  m3 <- lmer(value ~ QUESTION * STIMULUS + Study + (1|PID), data = df)
  summary(m3)
  anova(m3)
...
## (S3 2.2) REPORTED MODEL what stimulusX categorical questions change (in significant numbers)?
...
summary(mm.QS)
car::Anova(mm.QS)
emmeans(mm.QS, specs = c("QUESTION","STIMULUS"))


### REPORT THIS 
pigs.emm.s <- emmeans(mm.QS, specs = c("QUESTION","STIMULUS"))
pairs(pigs.emm.s)


mvcontrast(pigs.emm.s, "pairwise", mult.name = c("QUESTION", "STIMULUS"))
...
## REPORTED (S3 2.4) CHANGE IN SEMANTIC DIFFERENTIALS [RAW]
...
## WORKING SPARKTABLE S3 PRE/POST SDS & CONFIDENCE
...
## SANITY CHECK CONFIDENCE BY STUDY
```{r}
...
df %>% ggplot(aes(x=value, y=QUESTION, fill=TIME)) + 
  geom_density_ridges2(alpha=0.5, scale=1) + 
  facet_grid(. ~ STIMULUS)
##################################################  


######### JUST STUDY 1,2 BLOCK 1
df <- df_questions_long_all %>% 
  filter(Study %in% c("Study1","Study2")) %>% 
...
df %>% ggplot(aes(x=value, y=QUESTION, fill=Sample)) + 
  geom_density_ridges2(alpha=0.5, scale=1) + 
  facet_grid(. ~ STIMULUS)
##################################################  
