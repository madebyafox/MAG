...
df_sd_questions_abs_s3 <- readRDS("data/output/Study_3/df_sd_questions_abs_S3.rds")
df_sd_questions_long_z_S3 <- readRDS("data/output/Study_3/df_sd_questions_long_z_S3.rds")
...
## SETUP ATTITUDE CHANGE DATA 
```{r setup-attitude-change-data}
...
## (S3 2.1) Proportion of Participants who changed categoricals  (annotations IN FIGURE)
```{r}

# SETUP DATA
df <- df_change_all %>% 
  filter(QUESTION %in% ref_min_cat_questions) %>% 
  droplevels()


## 1. How many ppl changed for each stimulus x question 
table(df$QUESTION, df$SAME, df$STIMULUS)/40
...
## (S3 2.2) MODEL what stimulusX categorical questions change (in significant numbers)?
```{r s3-attitude-change-model}

# SETUP DATA
df <- df_change_all %>% 
  filter(QUESTION %in% ref_min_cat_questions) %>% 
  mutate(QUESTION = factor(QUESTION, levels = c("AGE","ID","GENDER","ENCOUNTER"))) %>% #reset factor levels for modelling b/c age is not sig
  mutate(STIMULUS = factor(STIMULUS, levels=c("B1-2","B1-1","B1-3","B1-4","B0-0"))) %>% 
  droplevels()

## sanity check 
print("data in this model")
table(df$Study, df$STIMULUS)

# PRE-REQ CHECKS  :::::::::::::::::::::::::::::::::::::

## check for sparse cells
table(df$QUESTION, df$SAME, df$STIMULUS)
levels(df$SAME)
levels(df$QUESTION)
levels(df$STIMULUS)


# MODEL PREDICTING CHANGE AT ALL :::::::::::::::::::::::::::::::::::::

#:: RANDOM INTERCEPT SUBJECT
mm.rS <- glmer(SAME ~ (1|PID), data = df,family = "binomial")
summary(mm.rS)

#:: QUESTION X STIMULUS
mm.QS <- glmer(SAME ~ QUESTION * STIMULUS + (1|PID), data = df,family = "binomial", 
...
#: 3 TEST SUPERIOR FIT
test_lrt(mm.rS,mm.QS) #same as anova
paste("Likelihood Ratio test is significant? p = ",(test_lrt(mm.QS,mm.rS))$p[2])
...
```
TODO REPORT:: 
A logistics mixed effects regression model predicting change in value by an interaction between QUESTION and STIMULUS (with participant as a random effect) indicated a significant main effect of ID, and interaction between gender and B1-1, indicating that most frequently indivdiual's changed their answer to the MAKER ID question, and also there were a lot of changes (45%) of participants changed their identification of maker gender when seeing the data from B1-1.  

SIGNIFICANT CHANGES IN CATEGORICALS
1. main effect MAKERID
2. sig ixn GENDER::B1-1 




##(S3 2.3) Categoricals and CONFIDENCE
...
## SETUP DATA
df <- df_graphs_z_s3 %>% 
  select("PID", "Study","Assigned.Block","STIMULUS","PRE_ID", "POST_ID","PRE_ID_CONF_z","POST_ID_CONF_z") %>%
  mutate(
    STIMULUS = factor(STIMULUS, levels = c("B1-2","B1-1","B1-3","B1-4","B0-0")),
    SAME = PRE_ID == POST_ID, 
    SAME = factor(SAME, levels = c("TRUE","FALSE")),
    SHIFT = as.numeric(POST_ID_CONF_z - PRE_ID_CONF_z)
)


## MODEL EFFECT OF STIMULUS ON AND CHANGE ON ID CONFIDENCE 
m <- lmer(SHIFT ~ STIMULUS+SAME + (1|PID), data = df)
summary(m)
anova(m)

## try with sum coding (comparing to grand mean rather than reference level as first categorical)
m2 <- lmer(SHIFT ~ STIMULUS+SAME + (1|PID), data = df, 
      # contrasts = list(STIMULUS = contr.sum, SAME = contr.sum)
      )

summary(m2)
## get the colnames from R
# colnames(model.matrix(m2))
# contrasts(df$SAME)
# anova(m2)
...
# 
# anova(m,m2)
# compare_performance(m,m2, rank = TRUE)

plot_model(m, show.intercept = TRUE, show.p = TRUE, show.values = TRUE)
plot_model(m, type = "pred", terms=c("STIMULUS","SAME"), show.data = FALSE)
plot_model(m, type = "pred", terms=c("STIMULUS"), show.data = TRUE)
plot_model(m, type = "pred", terms=c("SAME"), show.data = TRUE)
...
A linear mixed effects model predicting change in maker confidence by stimulus and change in maker showed that changes a significant main effect of both predictors, such that confidence slightly increased for each stimulus when the maker ID changed as well.  
...
## SETUP DATA
df <- df_graphs_z_s3 %>% 
  select("PID", "Study","Assigned.Block","STIMULUS","PRE_AGE", "POST_AGE","PRE_AGE_CONF_z","POST_AGE_CONF_z") %>%
  mutate(
    STIMULUS = factor(STIMULUS, levels = c("B1-2","B1-1","B1-3","B1-4","B0-0")),
    SAME = PRE_AGE == POST_AGE, 
    SAME = factor(SAME, levels = c("TRUE","FALSE")),
    SHIFT = as.numeric(POST_AGE_CONF_z - PRE_AGE_CONF_z)
)


## MODEL EFFECT OF STIMULUS ON AND CHANGE ON AGE CONFIDENCE 
m <- lmer(SHIFT ~ STIMULUS+SAME + (1|PID), data = df)
summary(m)
anova(m)
...
A linear mixed effects model predicting change in AGE confidence by stimulus and change in AGE showed NEITHER predictor was significant. 
...
## SETUP DATA
df <- df_graphs_z_s3 %>% 
  select("PID", "Study","Assigned.Block","STIMULUS",
         "PRE_GENDER", "POST_GENDER","PRE_GENDER_CONF_z","POST_GENDER_CONF_z") %>%
  mutate(
    STIMULUS = factor(STIMULUS, levels = c("B1-2","B1-1","B1-3","B1-4","B0-0")),
    SAME = PRE_GENDER == POST_GENDER, 
    SAME = factor(SAME, levels = c("TRUE","FALSE")),
    SHIFT = as.numeric(POST_GENDER_CONF_z - PRE_GENDER_CONF_z)
)
...
## (S3 2.4) CHANGE IN SEMANTIC DIFFERENTIALS [RAW]
...
## (S3 2.5) CHANGE IN SEMANTIC DIFFERENTIAL (ABS)
...
df_sd <- df_questions_s3 %>% 
  ...
  mutate(
      ...
      ## TRUE ABS CALC (absolute value difference) -- MAGNITUDE OF DIFF
       ## calculate change
      CHANGE_RAW = POST - PRE,
      ...
      ## DIRECTIONAL ABS CALC - towards neutral + toward 
      abs_post = abs(POST-50),
      abs_pre = abs(PRE-50),
      abs_change = abs_post - abs_pre
  )
...
##### USING INCORRECT ABSOLUTE VALUE CALCULATION 
## linear model
...
################### CALCULATE USING TRUE ABS VALUE, MAGNITUDE OF CHANGE ONLY 
# ixn model
 ...
 ## USE EMMEANS TO GET MORE INTERPRETABLE COEFFICIENTS
 library(emmeans)
 contrast(emmeans(m.2, ~ QUESTION * STIMULUS), method = "eff")
 contrast(emmeans(m.2, ~ QUESTION), method = "eff")
 contrast(emmeans(m.2, ~ STIMULUS), method = "eff")



############## CALCULATE USING THE OTHER ABSOLUTE VALUE APPROACH — WHERE - IS TOWARD NEUTRAL POSITIVE TOWARD POLES
## ixn model
f.2 <- "CHANGE ~ QUESTION * STIMULUS"
m.2 <- lmer(data = df, abs_change ~ QUESTION * STIMULUS + (1|PID))
summary(m.2)  
anova(m.2)
performance(m.2)
plot_model(m.2, type = "pred", terms=c("QUESTION","STIMULUS")) + labs(title = f.2)
...
gf_histogram(data = df, ~df$CHANGE_ABS)
gf_histogram(data = df, ~df$abs_change)
...
## xx WIP CONFIDENCE BY BLOCK 1 STIM BY STUDY 3 PRE/POST
...
all_q <- c(ref_min_conf_questions,ref_min_sd_questions)
...
for (s in row_names) { #ROWS ARE QUESTIONS
  ...
  for (q in col_names) { #COLS ARE STIMULI
    ...
    f[[s]][[q]] <- df %>% filter(STIMULUS==s) %>% select(TIME,q) 
  }
}
################################################
...
######### ROW TRACKING FUNCTION
# make_row_tracking_fun <- function(rows, full_data) {
#   counter <- 0
#   function(x, ...) {
#     counter <<- counter + 1
#     current_index <- rows[counter]
#     amy(x = x, row_index = current_index, full_data = full_data, ...)
#   }
# }
...
#### SETUP TABLE (FULL SDS)
tab2 <- data.frame(
  VARIABLE = all_q,
  LABEL = c("Maker Confidence", "Age Confidence","Gender Confidence","DESIGN", "DATA", "POLITICS","TRUST","ALIGN","BEAUTY","TRUST"),
  AGGREGATE = "",
  B1_A="",
  B1_B="",
  B1_C="",
...
t <- tinytable::tt(tab2, theme = "bootstrap") %>%
  ## PLOT AGGREGATE PLOTS IN COLUMN 2
  plot_tt(j=3, i= rows, fun=dist, data = df_num, color="black") %>% 
# %>% 
  ## PLOT B1_A IN COLUMN 3
  plot_tt(j=4, i=rows, fun = make_row_tracking_fun(rows, f[["B1-1"]],tab2), data = f[["B1-1"]]) %>%
  plot_tt(j=5, i=rows, fun = make_row_tracking_fun(rows, f[["B1-2"]],tab2), data = f[["B1-2"]]) %>%
  plot_tt(j=6, i=rows, fun = make_row_tracking_fun(rows, f[["B1-3"]],tab2), data = f[["B1-3"]]) %>% 
  plot_tt(j=7, i=rows, fun = make_row_tracking_fun(rows, f[["B1-4"]],tab2), data = f[["B1-4"]]) %>%
  plot_tt(j=8, i=rows, fun = make_row_tracking_fun(rows, f[["B0-0"]],tab2), data = f[["B0-0"]])
...
## WORKING CONFIDENCE BY BLOCK 1 STIM BY STUDY 1,2
...
## WORKING CONFIDENCE SPARKLINES BY STUDY
```{r sparktabletest}
...
## DECIDE DATAFRAME VERSION all studies; all samples, only conf
df <- df_graphs_all %>% 
  filter(
    #filter for only block 1 data
    # Assigned.Block==1, 
  ...
  #drop z-score cols
  select(Study,Sample,STIMULUS,TIME,ref_min_conf_questions) %>% 
   mutate(
...
## SETUP NUMERIC DATAFRAME
df_num <- df %>% select(Study,STIMULUS,where(is.numeric))
...
############ FILTER SIMPLE DATAFRAMES 
df_all <- df %>% select(ID_CONF,AGE_CONF,GENDER_CONF)
df_1 <- df %>% filter(Study=="Study1") %>% select(ID_CONF,AGE_CONF,GENDER_CONF)
df_2 <- df %>% filter(Study=="Study2") %>% select(ID_CONF,AGE_CONF,GENDER_CONF)
df_3 <- df %>% filter(Study=="Study3") %>% select(ID_CONF,AGE_CONF,GENDER_CONF)


############ POPULATE LIST OF FILTERED DATAFRAMES NUMERIC QUESTIONS FOR STUDY 3 THAT INCLUDES FACET
d_q <- c(ref_min_conf_questions)
# Define row and column names
col_names <- d_q 
# Initialize an empty list to store the structure
...
for (q in col_names) { 
  # Initialize an empty list for each row
  f[[q]] <- df %>% filter(Study=="Study3") %>% select(Sample,q) 
}
################################################
...
##WORKING FACETED
# amy <- function(x, row_index, full_data,...) {
#   # browser()
#   # 'x' is a vector (extracted column)
#   # 'row_index' is the index passed by plot_tt()
#   # 'full_data' is the original list of dataframes
#   
#   # Reconstruct dataframe for the given row index
#   vf <- full_data[[row_index]]
#   ggplot(vf, aes(x = DATA, fill = Study)) + 
#     geom_density(alpha=0.5) + 
#     theme_void() + easy_remove_legend()
# }
...
amy <- function(x, question_name, full_data,...) {
  ...
  vf <- full_data[[question_name]]
  # browser()
    ggplot(vf, aes(x = vf[[2]], fill = Sample)) +
    geom_density(alpha=0.5) +
...
############## TINY TABLE
### themes: bootstrap, grid, spacing
t <- tinytable::tt(tab, theme = "grid") %>%
  
  ## PLOT AGGREGATE PLOTS IN COLUMN 2
  plot_tt(j=2, i= 1:3, fun=dist, data = df_all, color="black") %>% 
  ## PLOT BY STUDY
  plot_tt(j=3, i= 1:3, fun=dist, data = df_1, color="black") %>% 
  plot_tt(j=4, i= 1:3, fun=dist, data = df_2, color="black") %>% 
  plot_tt(j=5, i=rows, fun = make_row_tracking_fun(rows, f,tab), data = f)
...
## WORKING PARTIAL SPARKLINES TABLE
...
## SETUP LIST OF NUMERIC DATAFRAMES 
all_q <- c(ref_min_cat_questions, ref_min_conf_questions,ref_min_sd_questions)
...
############ POPULATE LIST OF FILTERED DATAFRAMES NUMERIC QUESTIONS
d_q <- c(ref_min_conf_questions,ref_min_sd_questions)
stimuli <- as.vector(levels(df$STIMULUS))
...
for (s in row_names) { #ROWS ARE QUESTIONS
  ...
  for (q in col_names) { #COLS ARE STIMULI
    ...
    # m[[r]][[c]] <- data.frame(Value = sample(1:10, 5, replace = TRUE))
    m[[s]][[q]] <- df %>% filter(STIMULUS==s) %>% select(q) %>% pull()
    f[[s]][[q]] <- df %>% filter(STIMULUS==s) %>% select(Study,Sample,q) 
  }
}
################################################
...
#   # study <-  d[[1]]
#   # value <-  d[[2]]
#   # z <- bind_cols(study=study,value=value)
#   # head(z)
#   z <- data.frame(d)
#   browser()
#   z %>% 
...
amy <- function(x, question_name, full_data,...) {
  ...
  vf <- full_data[[question_name]]
  # browser()
  ggplot(vf, aes(x = vf[[3]], y = Study, fill = Sample)) +
      geom_density_ridges2(scale = 0.85, panel_scaling = TRUE, rel_min_height = 0.01, alpha = 0.25,linewidth=0.25 ) +
...
## AGGREGATE DATA ALL STIMULI
df_num <- df_num %>% select(where(is.numeric))

#   style_tt(j=2, align="c") 
# theme options: void, grid, bootstrap, spacing
...
############## TINY TABLE
### themes: bootstrap, grid, spacing
t <- tinytable::tt(tab, theme = "grid", width=c(3,2,1,1,1,1,1)) %>%
  ## PLOT AGGREGATE PLOTS IN COLUMN 2
  plot_tt(j=2, i= 1:4, fun=g, data = df_cat, height = 1) %>% #CATEGORICALS
  plot_tt(j=2, i= 5:14, fun=dist, data = df_num, color="black") %>% #NUMERIC
  
  ## PLOT B1_A IN COLUMN 3
  plot_tt(height=3,j=3, i=rows, fun = make_row_tracking_fun(rows, f[["B1-1"]],tab), data = f[["B1-1"]])%>%
  plot_tt(height=3,j=4, i=rows, fun = make_row_tracking_fun(rows, f[["B1-2"]],tab), data = f[["B1-2"]]) %>%
  plot_tt(height=3,j=5, i=rows, fun = make_row_tracking_fun(rows, f[["B1-3"]],tab), data = f[["B1-3"]]) %>%
  plot_tt(height=3,j=6, i=rows, fun = make_row_tracking_fun(rows, f[["B1-4"]],tab), data = f[["B1-4"]]) %>%
  plot_tt(height=3,j=7, i=rows, fun = make_row_tracking_fun(rows, f[["B0-0"]],tab), data = f[["B0-0"]])
  

  ### WORKING AGGREGATED ACROSS STUDIES  
  # plot_tt(j=3, i= 5:14, fun=dist, data = m[["B1-1"]], color="black") %>%
  # plot_tt(j=4, i= 5:14, fun=dist, data = m[["B1-2"]], color="black") %>%
  # plot_tt(j=5, i= 5:14, fun=dist, data = m[["B1-3"]], color="black") %>%
  # plot_tt(j=6, i= 5:14, fun=dist, data = m[["B1-4"]], color="black") %>%
  # plot_tt(j=7, i= 5:14, fun=dist, data = m[["B0-0"]], color="black")
  
  
  # plot_tt(j=3, i= 1:4, fun=g, data = df_cat, height = 1) %>% #CATEGORICALS
  # plot_tt(j=4, i= 5:14, fun=dist, (df_num %>% filter(STIMULUS=="B1-1") %>%
                                     # select(where(is.numeric))), color="black") %>%
  
  # ## PLOT B1_B IN COLUMN 3
  # # plot_tt(j=3, i= 1:4, fun=g, data = df_cat, height = 1) %>% #CATEGORICALS
  # plot_tt(j=4, i= 5:14, fun=dist, (df_num %>% filter(STIMULUS=="B1-2") %>% 
  #                                    select(where(is.numeric))), color="black") %>% #NUMERIC
  # 
  # ## PLOT B1_C IN COLUMN 3
  # # plot_tt(j=3, i= 1:4, fun=g, data = df_cat, height = 1) %>% #CATEGORICALS
  # plot_tt(j=5, i= 5:14, fun=dist, (df_num %>% filter(STIMULUS=="B1-3") %>% 
  #                                    select(where(is.numeric))), color="black") %>% #NUMERIC
  # 
  # ## PLOT B1_D IN COLUMN 3
  # # plot_tt(j=3, i= 1:4, fun=g, data = df_cat, height = 1) %>% #CATEGORICALS
  # plot_tt(j=6, i= 5:14, fun=dist, (df_num %>% filter(STIMULUS=="B1-4") %>% 
  #                                    select(where(is.numeric))), color="black") %>% #NUMERIC
  # 
  # ## PLOT B1_A IN COLUMN 3
  # # plot_tt(j=3, i= 1:4, fun=g, data = df_cat, height = 1) %>% #CATEGORICALS
  # plot_tt(j=7, i= 5:14, fun=dist, (df_num %>% filter(STIMULUS=="B0-0") %>% 
  #                                    select(where(is.numeric))), color="black") %>% #NUMERIC
  # 
  
  
  ##built in plot types: "histogram","density,"bar,line
  # style_tt(j=2, align="c")
...
################## FIT MODEL
f.all <-  "TRUST_z ~ (ALL SDS) + (1|PID)"
mm.all <- lmer(TRUST_z ~ BEAUTY_z * INTENT_z + INTENT_z*ALIGN_z + DATA_z + POLITICS_z+ DESIGN_z + ID + AGE + GENDER + (1|PID), data = df)
summary(mm.all)
car::Anova(mm.all) 

################## COMPARE MODEL
compare_performance(mm.all, mm.BxIxAD, rank = TRUE)
anova(mm.all, mm.BxIxAD)

```
- In a model with simple main effects of all predictors (including categoricals), ID and POLITICS are also significant. However, we believe some of the variance in ID is accounted for by INTENT, and variance in POLITICS is better represented in ALIGNMENT

Even though this model is a marginally (though significantly) better fit, we shoose to go with the simpler model
