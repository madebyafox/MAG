...
This file takes raw data files from (3) Qualtrics surveys (Study 1, Study 2, Study 3), standardizing their format for further data analysis. Note that all raw data comes from Qualtrics, and each Qualtrics survey includes a different stimulus block (set of 5 stimuli).
...
4.  *Data for Study 4 (a repeated measures design where participants rate the stimuli with obscured and unobscured versions of each image)*
...
# order_study = c("TUMBLR","PROLIFIC") ##NEW for VIS
...
## IMPORT RAW Participant Data Files
...
##CHECK number per block per sample 
print("Number of valid participants per assigned block and sample")
table(df_data$Sample, df_data$Assigned.Block)

##CHECK number per block per study 
print("Number of valid participants per assigned block and study")
table(df_data$Study, df_data$Assigned.Block)
...
print("df_data represents full set of RESPONSE DATA for valid  participants [wide]")
print("df_participants represents full set of DEMOGRAPHIC DATA for valid participants [demographics]")
...
*Here we pivot response data from valid participants to be represented at the QUESTION (i.e. semantic differential) and TRIAL (ie. question+stimulus) levels.*
...
df_graphs_z <- df_graphs %>% 
  mutate(
    ...
    MAKER_CONF = datawizard::standardize(MAKER_CONF),
    AGE_CONF = datawizard::standardize(AGE_CONF),
    GENDER_CONF = datawizard::standardize(GENDER_CONF),
    TOOL_CONF = datawizard::standardize(TOOL_CONF)
  )
