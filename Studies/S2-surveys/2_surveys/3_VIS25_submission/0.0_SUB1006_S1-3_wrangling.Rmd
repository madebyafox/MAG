---
title: "S1-3 DATA WRANGLING—SUBMISSION 1006"
author: "ANONYMIZED"
date: "2025-02-24"
output:
  html_document:
...
This notebook takes raw data files from (3) Qualtrics surveys (Study 1, Study 2, Study 3), standardizing their format for further data analysis. Note that all raw data comes from Qualtrics, and each Qualtrics survey includes a different stimulus block (set of 5 stimuli).

1.  **Study 1:** We collected data sampling directly from the Tumblr social media platform produces files with slightly different metadata (several fields required for tracking Prolific are not included). The Qualtrics surveys for Tumblr used randomization logic to assign each Tumblr participant to one of the six stimulus blocks. **(All stimulus blocks, n \~ 12-15 / block)**

2.  **Study 2:** Initial data collection via PROFLIFIC occurred via Qualtrics surveys named `MAG_S2_PROLIFIC-DATACOLLAR_0` and `MAG_S2_PROLIFIC-DATACOLLAR_0`, each presenting participants with one common graph ("block 0", `STIMULUS = B0-0`) and four subsequent graphs in random order ("block 1"). These data were collected via Prolific, with different demographic restrictions (i.e. 'data collar' and 'blue collar' made use of different values in the Prolific participant background survey on the 'technology use at work' and 'job title' questions.) However, when we duplicated these questions in our own demographic surveys, we found participants' answers often did not match their values on the Prolific background survey, and thus we abandoned this stratified sampling strategy and continued data collection with no demographic restrictions. **(Stimulus block #1 only; n = 40 (20 blue-collar; 20 data-collar) )**

3.  **Study 3:** General (non-stratified) sampling via PROLIFIC collecting data for all 6 stimulus blocks. **(n =40/block )**

4.  *Data for Study 4 (a repeated measures design where participants rate the stimuli with obscured and unobscured versions of each image)* This wrangling is handled in a separate notebook.
...
############## LABELS FOR Z-scored VALUE OF SD QUESTIONS
...
order_study = c("Study1","Study2","Study3","Study4") ##NEW for VIS
...
####WRANGLE MASTER PARTICIPANT-LEVEL DF  #####################################################
...
df_exclude <- df_data %>% 
  ...
  select(
    ...
    D_gender:D_politicsFiscal, SCREEN_workMethod:FEEDBACK, Prolific.Name:Study
  )  %>% 
  mutate(
...
write.csv(df_exclude, file = "data/output/Study_1_2_3/df_exclude.csv", na="")
saveRDS(df_exclude, file = "data/output/Study_1_2_3/df_exclude.rds")
...
cols = c("Block","Study1","Study2","Study3","Study4","sum")
...
title = "Number of successful surveys by sample and block"
cols = c("Sample", "Block-1","Block-2","Block-3","Block-4","Block-5","Block-6", "Sum")
cont <- table(df_data$Sample, df_data$Assigned.Block)
cont %>% addmargins() %>% kbl(caption = title, col.names = cols) %>%  kable_classic()

title = "Number of successful surveys by study and block"
...
write.csv(df_data, file = "data/output/Study_1_2_3/df_data.csv", na="")
saveRDS(df_data, file = "data/output/Study_1_2_3/df_data.rds")

```

## Create DEMOGRAPHICS file

`df_participants` represents **demographic data** for all valid participants (WIDE)
...
write.csv(df_participants, file = "data/output/Study_1_2_3/df_participants.csv", na="")
saveRDS(df_participants, file = "data/output/Study_1_2_3/df_participants.rds")

```

## Create QDA WIDE

Two files are created for the purpose of qualitative data analysis: - `df_qda_wide`: PARTICIPANT level: includes all valid responses in wide format, to be used for free response data analysis (wide) - `df_qda_long`: GRAPH level: represents response and demographic data for all valid participants (used to generate a spreadsheet for analysis for free response data) *at the trial (i.e. stimulus) level* (i.e. long).
...
df_qda_wide <- df_data
write.csv(df_data, file = "data/output/Study_1_2_3/df_qda_wide.csv", na="")
...
write.csv(df_qda_long, file = "data/output/Study_1_2_3/df_qda_long.csv", na="")

```

## Create QUESTION level (long)

`df_questions` is at the graph-question level, but includes *all* questions, both categorical, free response, and numeric.
...
df_questions <- df_data %>% 
  ...
  select(-stimulus) %>% #drop unformatted stimulus column
  filter(QUESTION !="loop_number") %>%  #drop loop number rows, we don't use this question
  ## !! FILTER OUT BLANK ROWS (i.e otherwise there is a row for each stimulus in the blocks the person was not assigned to)
  filter(as.character(BLOCK)==as.character(tempblock) | BLOCK=="B0") %>% 
  select(-tempblock) #drop tempblock column



## WRITE DATAFRAME
write.csv(df_questions, file = "data/output/Study_1_2_3/df_questions_long.csv", na="")
saveRDS(df_questions, file = "data/output/Study_1_2_3/df_questions_long.rds")
```
...
## WRITE DATAFRAME
write.csv(df_sd_questions_long, file = "data/output/Study_1_2_3/df_sd_questions_long.csv", na="")
saveRDS(df_sd_questions_long, file = "data/output/Study_1_2_3/df_sd_questions_long.rds")
...
## WRITE DATAFRAME
write.csv(df_sd_questions_wide, file = "data/output/Study_1_2_3/df_sd_questions_wide.csv", na="")
saveRDS(df_sd_questions_wide, file = "data/output/Study_1_2_3/df_sd_questions_wide.rds")
```

## Create TOOLS

`df_tools` contains 1 row per PARTICIPANT-GRAPH-TOOL (tool is a multi-select question)
...
## WRITE DATAFRAME
write.csv(df_tools, file = "data/output/Study_1_2_3/df_tools.csv", na="")
saveRDS(df_tools, file = "data/output/Study_1_2_3/df_tools.rds")
```

## Create ACTIONS

`df_actions` contains 1 row per PARTICIPANT-GRAPH-ACTION (tool is a multi-select question)
...
## WRITE DATAFRAME
write.csv(df_actions, file = "data/output/Study_1_2_3/df_actions.csv", na="")
saveRDS(df_actions, file = "data/output/Study_1_2_3/df_actions.rds")
```
...
## WRITE DATAFRAME
write.csv(df_graphs_full, file = "data/output/Study_1_2_3/df_graphs_full.csv", na="")
saveRDS(df_graphs_full, file = "data/output/Study_1_2_3/df_graphs_full.rds")

```

## Create GRAPH/TRIAL level (SD and categorical)

`df_graphs_full` contains 1 row / participant-graph (all qs as columns), only for the SD and categorical (not multi-select or free response questions).

```{r GRAPH-LEVEL-MAIN}

#### DFS FOR TRIAL LEVEL ANALYSIS w/o free response ######################################################
#### CHART LEVEL DATA FRAME (LONG) ##########################
...
## WRITE DATAFRAME
write.csv(df_graphs, file = "data/output/Study_1_2_3/df_graphs.csv", na="")
saveRDS(df_graphs, file = "data/output/Study_1_2_3/df_graphs.rds")
...
df_graphs_z <- df_graphs %>% 
  mutate(
    MAKER_DESIGN_z  = datawizard::standardize(MAKER_DESIGN ),
    MAKER_DATA_z = datawizard::standardize(MAKER_DATA),
    MAKER_POLITIC_z = datawizard::standardize(MAKER_POLITIC),
    MAKER_ARGUE_z = datawizard::standardize(MAKER_ARGUE),
    MAKER_SELF_z = datawizard::standardize(MAKER_SELF),
    MAKER_ALIGN_z = datawizard::standardize(MAKER_ALIGN),
    MAKER_TRUST_z = datawizard::standardize(MAKER_TRUST),
    CHART_LIKE_z = datawizard::standardize(CHART_LIKE),
    CHART_BEAUTY_z = datawizard::standardize(CHART_BEAUTY),
    CHART_INTENT_z = datawizard::standardize(CHART_INTENT),
    CHART_TRUST_z = datawizard::standardize(CHART_TRUST),
    MAKER_CONF_z = datawizard::standardize(MAKER_CONF),
    AGE_CONF_z = datawizard::standardize(AGE_CONF),
    GENDER_CONF_z = datawizard::standardize(GENDER_CONF),
    TOOL_CONF_z = datawizard::standardize(TOOL_CONF)
  )
write.csv(df_graphs_z, file = "data/output/Study_1_2_3/df_graphs_z.csv", na="")
saveRDS(df_graphs_z, file = "data/output/Study_1_2_3/df_graphs_z.rds")
...
df_sd_questions_long_z <- df_graphs_z %>% 
  ...
  mutate(
    MAKER_DESIGN_z = as.numeric(MAKER_DESIGN_z),
    MAKER_DATA_z = as.numeric(MAKER_DATA_z),
    MAKER_POLITIC_z = as.numeric(MAKER_POLITIC_z),
    MAKER_ARGUE_z = as.numeric(MAKER_ARGUE_z),
    MAKER_SELF_z = as.numeric(MAKER_SELF_z),
    MAKER_ALIGN_z = as.numeric(MAKER_ALIGN_z),
    MAKER_TRUST_z = as.numeric(MAKER_TRUST_z),
    CHART_LIKE_z = as.numeric(CHART_LIKE_z),
    CHART_BEAUTY_z = as.numeric(CHART_BEAUTY_z),
    CHART_INTENT_z = as.numeric(CHART_INTENT_z),
    CHART_TRUST_z = as.numeric(CHART_TRUST_z)) %>% 
  #drop non z-scored columns AND confidence cols
  select(- (ENCOUNTER:CHART_LATENCY), 
         - contains("_CONF"))%>% 
  ## pivot longer the SD questions
  pivot_longer(
    cols = ref_sd_questions_z,
    names_to = "QUESTION"
  ) %>% mutate(
    QUESTION = factor(QUESTION, levels=ref_sd_questions_z)
  )
write.csv(df_sd_questions_long_z, file = "data/output/Study_1_2_3/df_sd_questions_long_z.csv", na="")
saveRDS(df_sd_questions_long_z, file = "data/output/Study_1_2_3/df_sd_questions_long_z.rds")    
...
Create duplicate versions of datasets used to analyze the semantic differential scores with absolute values (i.e. folding SD scale in half, so 0 = center of the slider, and 50 = the left and right extremes) Created as separate files rather than adding cols to extant dfs b/c it makes the looping graphing code more readable
...
df_graphs_abs <- df_graphs %>% 
  mutate(
    ...
    MAKER_CONF =  abs(MAKER_CONF - 50),
    AGE_CONF =  abs(AGE_CONF - 50),
    GENDER_CONF =  abs(GENDER_CONF - 50),
    TOOL_CONF =  abs(TOOL_CONF - 50)
    
  )
write.csv(df_graphs_abs, file = "data/output/Study_1_2_3/df_graphs_abs.csv", na="")
saveRDS(df_graphs_abs, file = "data/output/Study_1_2_3/df_graphs_abs.rds")
...
write.csv(df_sd_questions_long_abs, file = "data/output/Study_1_2_3/df_sd_questions_long_abs.csv", na="")
saveRDS(df_sd_questions_long_abs, file = "data/output/Study_1_2_3/df_sd_questions_long_abs.rds")    
