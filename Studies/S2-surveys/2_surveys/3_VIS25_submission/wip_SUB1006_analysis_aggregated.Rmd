---
title: "SUBMISSION 1006 — AGGREGATED DATA"
author: "ANONYMIZED"
date: "2024-02-24"
output:
  html_document:
...
## DECIDE DATAFRAME VERSION (Raw, z-scored, abs)
df <- df_graphs
...
B <-
  ...
  facet_grid(.~Sample) + 
  theme_minimal() + 
  labs(title = "Confidence by Survey Question and Sample", caption = "(mean in blue)")

  
## R
## CONFIDENCE BY QUESTION & SAMPLE (all stimuli, all Pps, by stdy)
R <- 
  df %>% 
  ggplot(aes(x=CONFIDENCE, y=fct_rev(QUESTION), fill = Sample)) + #fill=fct_rev(QUESTION))) + 
    geom_density_ridges( scale = 0.65, alpha = 0.5, quantile_lines = TRUE,   
  ...
  ## MEAN
    ...
    # stat_summary(fun=mean, geom="point", size=2, color="blue", fill="blue") +
  theme_minimal() + 
  labs(title = "Confidence by Survey Question and Sample", y = "QUESTION", caption ="") 

  
(B+R)
if(GRAPH_SAVE){ggsave(plot = (B+R), path="figs/level_aggregated/distributions", filename =paste0("all_confidence_by_sample.png"), units = c("in"), width = 14, height = 8 ,  bg='#ffffff'  )}
...
## D
## MAKER IDENTIFICATION AGGREGATED (by Sample)
## GGSTATSPLOT
##############################
#hack for consistent ordering of ggstats bar plot
dx <- df %>% mutate( MAKER_ID = fct_rev(MAKER_ID) )
S <-   ggbarstats( data = dx, x = MAKER_ID, y = Distribution,
...
##############################
H <- df %>% 
  group_by(MAKER_ID,Distribution) %>% 
...
if(GRAPH_SAVE){ggsave(plot = p, path="figs/level_aggregated/categoricals", filename =paste0("all_maker_id_by_sample.png"), units = c("in"), width = 14, height = 8 ,  bg='#ffffff'  )}
...
S <-   ggbarstats( data = dx, x = MAKER_AGE, y = Distribution,
...
##############################
H <- df %>% 
  group_by(MAKER_AGE,Distribution) %>% 
...
if(GRAPH_SAVE){ggsave(plot = p, path="figs/level_aggregated/categoricals", filename =paste0("all_maker_age_by_sample.png"), units = c("in"), width = 14, height = 8 ,  bg='#ffffff'  )}
...
### by Sample
```{r maker-gender-confidence-sample}
...
S <-   ggbarstats( data = dx, x = MAKER_GENDER, y = Distribution,
...
##############################
H <- df %>% 
  group_by(MAKER_GENDER,Distribution) %>% 
...
if(GRAPH_SAVE){ggsave(plot = p, path="figs/level_aggregated/categoricals", filename =paste0("all_maker_gender_by_sample.png"), units = c("in"), width = 14, height = 8 ,  bg='#ffffff'  )}
...
### by Sample
```{r maker-tool-confidence-sample}
...
##############################
#hack for consistent ordering of ggstats bar plot
S <-   ggbarstats( data = df, x = TOOL_ID, y = Distribution,
...
(p <- (S + H)) + plot_annotation(
  ...
  # subtitle = "the categories of MAKER ID were chosen in similar proportion, 
  # and both the mean (in blue) and shape of distribution of confidence scores is similar across values of Maker ID",
  caption = "(blue indicates mean); is multi-select option"
)

if(GRAPH_SAVE){ggsave(plot = p, path="figs/level_aggregated/categoricals", filename =paste0("all_tool_ID_by_sample.png"), units = c("in"), width = 14, height = 8 ,  bg='#ffffff'  )}
...
if(GRAPH_SAVE){ggsave(plot = p, path="figs/level_aggregated/categoricals", filename =paste0("all_encounter_by_sample.png"), units = c("in"), width = 14, height = 8 ,  bg='#ffffff'  )}
```
...
S <-   ggbarstats( data = dx, x = CHART_ACTION, y = Distribution,
                   legend.title = "ACTION",
                   results.subtitle = FALSE) + 
    scale_fill_manual(values = my_palettes(name="actions", direction = "1")) +
    theme_minimal() +
    labs( title = "CHART ACTION by SAMPLE",  x = "", y="") + 
    theme(aspect.ratio = 1)
##############################

if(GRAPH_SAVE){ggsave(plot = S, path="figs/level_aggregated/categoricals", filename =paste0("all_action_by_sample.png"), units = c("in"), width = 14, height = 8 ,  bg='#ffffff'  )}
S
...
S <-   ggbarstats( data = dx, x = CHART_ACTION4, y = Distribution,
                   legend.title = "ACTION",
                   results.subtitle = FALSE) + 
    scale_fill_manual(values = my_palettes(name="actions4", direction = "1")) +
    theme_minimal() +
    labs( title = "CHART ACTION [4] by SAMPLE",  x = "", y="") + 
    theme(aspect.ratio = 1)
##############################

if(GRAPH_SAVE){ggsave(plot = S, path="figs/level_aggregated/categoricals", filename =paste0("all_action4_by_sample.png"), units = c("in"), width = 14, height = 8 ,  bg='#ffffff'  )}
S
...
S <-   ggbarstats( data = dx, x = CHART_ACTION3, y = Distribution,
                   legend.title = "ACTION",
                   results.subtitle = FALSE) + 
    scale_fill_manual(values = my_palettes(name="actions3", direction = "1")) +
    theme_minimal() +
    labs( title = "CHART ACTION [3] by SAMPLE",  x = "", y="") + 
    theme(aspect.ratio = 1)
##############################

if(GRAPH_SAVE){ggsave(plot = S, path="figs/level_aggregated/categoricals", filename =paste0("all_action3_by_sample.png"), units = c("in"), width = 14, height = 8 ,  bg='#ffffff'  )}
S
...
S <-   ggbarstats( data = dx, x = CHART_ACTION2, y = Distribution,
                   legend.title = "ACTION",
                   results.subtitle = FALSE) + 
    scale_fill_manual(values = my_palettes(name="actions2", direction = "1")) +
    theme_minimal() +
    labs( title = "CHART ACTION [2] by SAMPLE",  x = "", y="") + 
    theme(aspect.ratio = 1)
##############################

if(GRAPH_SAVE){ggsave(plot = S, path="figs/level_aggregated/categoricals", filename =paste0("all_action2_by_sample.png"), units = c("in"), width = 14, height = 8 ,  bg='#ffffff'  )}
S
```


### Combined Sample
```{r action-choice-combined}
...
(p <- df_full %>% 
  ggbarstats(  
            x = PLATFORM, y = Distribution,
            legend.title = "PLATFORM",
            results.subtitle = FALSE) + 
    scale_fill_manual(values = my_palettes(name="platforms", direction = "-1"))+
    theme_minimal() + 
    labs( title = "PLATFORM Choice by SAMPLE ", subtitle = "", x = "")
)

if(GRAPH_SAVE){ggsave(plot = p, path="figs/level_aggregated/categoricals", filename =paste0("all_platform_choice_by_Sample.png"), units = c("in"), width = 14, height = 8 ,  bg='#ffffff'  )}
...
```{r plot_boxplot_sds_by_sample, fig.width=10, fig.height=14}
  ...
  plots <- as.list(lapply(colnames(cols), plot_sd, data = df, type ="S", mean=TRUE, facet = TRUE, facet_by = "Distribution", boxplot=TRUE, labels=ref_labels))
...
if(GRAPH_SAVE == TRUE){  
  ggsave(plot = plot_master_questions, path="figs/level_aggregated/distributions", filename =paste0("combined_stimuli_by_sample","_box.png"), units = c("in"), width = 10, height = 14  )
}
...
```{r plot_boxplot_sds_combined, fig.width=10, fig.height=14}
...
```{r plot_halfeye_sds_by_sample}
  ...
  labs (title = "ALL STIMULI by SAMPLE", y = "", caption = "(point is median)") +
  theme_minimal() + easy_remove_legend()
)

  if(GRAPH_SAVE == TRUE){ 
  ggsave(plot = g, path="figs/level_aggregated/distributions", filename =paste0("combined_stimuli_by_sample","_ggdist.png"), units = c("in"), width = 10, height = 14  )
  }
  

```

```{r plot_halfeye_sds_combined}
...
if(GRAPH_SAVE == TRUE) {
    ggsave(plot = x, path="figs/level_aggregated/distributions", filename =paste0("combined_stimuli_by_sample","_ridges.png"), units = c("in"), width = 10, height = 14  )
}


```    

```{r plot_ridglines_sds_combined}
...
```{r ALL-Q-COLLAPSED-ABS_by_sample, fig.width=10, fig.height=14}
...
if(GRAPH_SAVE == TRUE){  
  ggsave(plot = plot_master_questions, path="figs/level_aggregated/distributions", filename =paste0("ABS_combined_stimuli_by_sample","_box.png"), units = c("in"), width = 10, height = 14  )
}
...
##### ggdist halfeye 
```{r plot_ggdist_sd_abs_by_sample}
...
(  g <- ggplot(d, aes(y = fct_rev(QUESTION), x = value, fill=category)) +
  ...
  labs (title = "ALL STIMULI — SD (ABSOLUTE VALUE) — by SAMPLE", y = "", caption = "(point is median)") +
  theme_minimal() + easy_remove_legend()
) 

if(GRAPH_SAVE == TRUE){
  ggsave(plot = g, path="figs/level_aggregated/distributions", filename =paste0("ABS_combined_stimuli_by_sample","_ggdist.png"), units = c("in"), width = 10, height = 14  )
}
  
```

```{r plot_ggdist_sd_abs_combined}
...
##### density ridges
```{r plot_ridges_sd_abs_by_sample}
...
(  x <-ggplot(d, aes(x = value, y = fct_rev(QUESTION), fill = category)) +
    ...
    labs(title = "ALL STIMULI — SD (ABSOLUTE VALUE) — by Sample", y = "", caption = "(point is median)") +
    cowplot::draw_text(text = ref_sd_questions, x = 50, y= ref_sd_questions_abs, size = 8, vjust=-2, position=position_nudge(y=-.20))  + ##raw
    theme_minimal() + easy_remove_legend()
)

if(GRAPH_SAVE == TRUE){
    ggsave(plot = x, path="figs/level_aggregated/distributions", filename =paste0("ABS_combined_stimuli_by_sample","_ridges.png"), units = c("in"), width = 10, height = 14  )
}
  
```    

```{r plot_ridges_sd_abs_combined}
