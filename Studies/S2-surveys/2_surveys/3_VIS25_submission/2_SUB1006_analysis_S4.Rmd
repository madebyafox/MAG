---
title: "SUBMISSION 1006 — Study 4 Analysis"
author: "ANONYMIZED"
date: "2024-02-24"
output:
  html_document:
...
ref_labels <- readRDS("data/input/REFERENCE/ref_labels_S4.rds")
ref_labels_abs <- readRDS("data/input/REFERENCE/ref_labels_abs_S4.rds")
...
df_participants <- readRDS("data/output/Study_4/df_participants_S4.rds")

# 1 row per participant-graph 
df_graphs <- readRDS("data/output/Study_4/df_graphs_S4.rds")
# for modelling
df_graphs_z <- readRDS("data/output/Study_4/df_graphs_z_S4.rds")

# sd questions LONG (with pre/post in sep cols)
df_questions_long <- readRDS("data/output/Study_4/df_questions_long_S4.rds")
df_sd_questions_long <- readRDS("data/output/Study_4/df_sd_questions_long_S4.rds") #pre/post in sep rows as TIME
df_questions <- readRDS("data/output/Study_4/df_questions_S4.rds") #pre/post in sep cols


df_sd_questions_long_abs <-  readRDS("data/output/Study_4/df_sd_questions_long_abs_S4.rds")
...
# if(GRAPH_SAVE == TRUE) {
#     ggsave(plot = x, path="figs/study4", filename =paste0("S4_B1_SDS_by_Stimulus_ridges.png"), units = c("in"), width = 10, height = 14,  bg='#ffffff'  )}
```
...
(x <-
    ...
    cowplot::draw_text(text = ref_sd_questions_abs, x = 100, y= ref_sd_questions_abs,size = 8, vjust=-2, position=position_nudge(y=-.20))  + ##raw
...
if(GRAPH_SAVE == TRUE) {
    ggsave(plot = x, path="figs/study4", filename =paste0("S4_B1_SDS_by_Stimulus_ridges_ABS.png"), units = c("in"), width = 10, height = 14,  bg='#ffffff'  )}
```
...
if(GRAPH_SAVE == TRUE) {
    ggsave(plot = S, path="figs/study4", filename =paste0("S4_GENDER_by_STIMLUS.png"), units = c("in"), width = 16, height = 8,  bg='#ffffff'  )
  }
...
if(GRAPH_SAVE == TRUE) {
    ggsave(plot = S, path="figs/study4", filename =paste0("S4_ID_by_STIMLUS.png"), units = c("in"), width = 16, height = 8,  bg='#ffffff'  )
  }
```
...
if(GRAPH_SAVE == TRUE) {
    ggsave(plot = S, path="figs/study4", filename =paste0("S4_AGE_by_STIMLUS.png"), units = c("in"), width = 16, height = 8,  bg='#ffffff'  )
  }
...
if(GRAPH_SAVE == TRUE) {
    ggsave(plot = x, path="figs/study4", filename =paste0("S4_MATRIX_ID_Confidence_scatterplot.png"), units = c("in"), width = 10, height = 10,  bg='#ffffff'  )}
```
...
if(GRAPH_SAVE == TRUE) {
    ggsave(plot = x, path="figs/study4", filename =paste0("S4_MATRIX_ID_Confidence_parallelcords.png"), units = c("in"), width = 10, height = 10,  bg='#ffffff'  )}
...
if(GRAPH_SAVE == TRUE) {
    ggsave(plot = x, path="figs/study4", filename =paste0("S4_MATRIX_AGE_Confidence_scatterplot.png"), units = c("in"), width = 10, height = 10,  bg='#ffffff'  )}
```
...
if(GRAPH_SAVE == TRUE) {
    ggsave(plot = x, path="figs/study4", filename =paste0("S4_MATRIX_AGE_Confidence_parallelcords.png"), units = c("in"), width = 10, height = 10,  bg='#ffffff'  )}
...
if(GRAPH_SAVE == TRUE) {
    ggsave(plot = x, path="figs/study4", filename =paste0("S4_MATRIX_GENDER_Confidence_scatterplot.png"), units = c("in"), width = 10, height = 10,  bg='#ffffff'  )}
```
...
if(GRAPH_SAVE == TRUE) {
    ggsave(plot = x, path="figs/study4", filename =paste0("S4_MATRIX_GENDER_Confidence_parallelcords.png"), units = c("in"), width = 10, height = 10,  bg='#ffffff'  )}
...
# much more flexible than GGSANKEY, but more work required
# 
# 
# ## FILTER DATASET
# df <- df_questions_long %>% 
#   filter(QUESTION=="ID") %>% 
#   select(PID, STIMULUS, TIME, value) %>% 
#   mutate(
#     value = factor(value, levels = rev(order_maker))
#   )
# 
# ### {GGALLUVIAL} ################################
# 
...
# ## ALLUVIAL DIAGRAM
# 
# # are the data in lodes form?
# # is_lodes_form(df, key = "TIME", value = "value", id = "PID")
# df %>% ggplot(aes(alluvium = PID, x = TIME, stratum = value, fill = value)) +
#   geom_flow() + 
#   geom_stratum() + 
#   facet_grid(.~STIMULUS) + 
#   scale_fill_manual(values = my_palettes(name="reds", direction = "1"), guide = guide_legend(reverse = TRUE)) 
