...
ref_min_sd_questions <-  c("MAKER_DESIGN","MAKER_DATA","MAKER_POLITIC", "MAKER_ALIGN","CHART_INTENT","MAKER_TRUST","CHART_BEAUTY")  
ref_s3_sd_questions <-  c("DESIGN","DATA","POLITICS", "ALIGN","INTENT","TRUST","BEAUTY")  
...
# df_data <- readRDS("data/output/Study_1_2/df_data.rds")


# 1 row per participant — demographic
df_participants <- readRDS("data/output/Study_1_2/df_participants.rds")
df_participants_S3 <- readRDS("data/output/Study_3/df_participants_S3.rds")
df_participants_all <- readRDS("data/output/COMBINED/df_participants_ALL.rds")

# sd questions LONG
df_sd_questions_long <- readRDS("data/output/Study_1_2/df_sd_questions_long.rds")
df_sd_questions_long_S3 <- readRDS("data/output/Study_3/df_sd_questions_long_S3.rds")
df_sd_questions_long_all <- readRDS("data/output/COMBINED/df_sd_questions_long_ALL.rds")


## FILTER BLOCK1
# df_b1_sd_questions_long <- df_sd_questions_long %>% 
#   filter(Assigned.Block==1) %>% 
#   filter(QUESTION %in% ref_min_sd_questions) %>% droplevels()
#                                                                                          
# df_b1_sd_questions_long_S3 <- df_sd_questions_long_S3 %>%
#   filter(Assigned.Block==1) %>%
#   filter(TIME =="PRE") %>% select(-TIME) %>% droplevels()
# 
# ## THIS INCLUDES PRE/POST IN THE 'SAMPLE COLUMNS'
# df_b1_sd_questions_long <- rbind(df_b1_sd_questions_long, df_b1_sd_questions_long_S3) %>% 
#   mutate(
#     QUESTION = str_replace_all(QUESTION, "MAKER_POLITIC","POLITICS"),
#     QUESTION = str_replace_all(QUESTION, "MAKER_",""),
#     QUESTION = str_replace_all(QUESTION, "CHART_","")
#   )
# 
# 
# ### WRANGLE LONG DF WHERE S3 PRE/POST STORED IN SAMPLE
# df_b1_sd_questions_long_S3 <- df_sd_questions_long_S3 %>% 
#   filter(Assigned.Block==1) %>% 
#   mutate(Sample = TIME) %>% 
#   droplevels()
...
df_graphs <- readRDS("data/output/Study_1_2/df_graphs.rds")
...
## FILTER FOR BLOCK 1 PARTICIPANTS AND JOIN S1_2 & S3
# df_b1_participants <- df_participants %>% filter(Assigned.Block==1) %>% droplevels()
# df_b1_participants_S3 <- df_participants_S3 %>% filter(Assigned.Block==1) %>% droplevels()
# df_b1_participants <- rbind(df_b1_participants, df_b1_participants_S3)
# 
# 
...
table(df_b1_participants$Study, df_b1_participants$Assigned.Block)
...
## BLOCK 1 Question by Stimulus by Study (vertical facet)


### S1_S2_S3(PRE-POST) stacked density ridges
...
questions <- ref_s3_sd_questions #has qs in right order
...
### S1_S4 studies stacked density ridges
```{r plot_ridglines_sds_questions_stimulus_studies_block1}
  
#### DENSITY RIDGES#############################################################################
#### loop over questions and stimuli, vertically stack studies, color by sample

## DEFINE DF
df <- df_sd_questions_long_all %>% filter(Assigned.Block==1) %>% droplevels()
...
for (s in stimuli){
  ...
  for (q in questions) {
    ...
    x[[i]] <- 
      ggplot(d, aes(x = value, y = Study, fill = Sample, color = Sample, )) +
      geom_density_ridges2(scale = 1.5, panel_scaling = TRUE, rel_min_height = 0.01, alpha = 0.25,
          # ## POINT JITTER GEOMETRY
          # jittered_points = TRUE, alpha = 0.7, scale = 0.9)+
           # ## RUG GEOMETRY
            jittered_points = TRUE,
  ...
  ## SAVE GRAPH FOR THIS STIMULIS 
  if(GRAPH_SAVE == TRUE) {
     ggsave(plot = p, path="figs/level_block1/FIG_B1/S1_4", filename =paste0("FULL_STUDY_STACK_",s,".png"), units = c("in"), width = 8, height = 24,  bg='#ffffff'  )}
...
### S1_S3 studies stacked density ridges (PRE ONLY)

In this version, we don't use Study 2 pilot and rename S4 as S3
```{r plot_ridglines_sds_questions_stimulus_studies_block1}
...
## DEFINE DF
df <- df_sd_questions_long_all %>% filter(Assigned.Block==1) %>% droplevels()
# filter out Study 2 (data/blue collar pilot)
df <- df %>% filter(Study!="Study0") %>% droplevels()
...
for (s in stimuli){
  ...
  for (q in questions) {
    ...
    x[[i]] <- 
      ggplot(d, aes(x = value, y = Study, fill = Study, color = Study, )) +
      geom_density_ridges2(scale = 1.5, panel_scaling = TRUE, rel_min_height = 0.01, alpha = 0.25,
          # ## POINT JITTER GEOMETRY
          # jittered_points = TRUE, alpha = 0.7, scale = 0.9)+
           # ## RUG GEOMETRY
            ...
            point_shape = '|', point_size = 3, point_alpha = 0.5) +
      scale_x_continuous(limits=c(0,100)) +
      scale_fill_manual(values = my_palettes(name="studies", direction = "1")) +
      scale_color_manual(values = my_palettes(name="studies", direction = "1")) +
      ## MEDIAN
      stat_summary(fun=median, geom="text", fontface = "bold", size= 4,
                vjust=+2, hjust = 0.50, aes(label=round(m, digits=0)))+
      stat_summary(fun=median, geom="point", size=2) +
  ...
  ## SAVE GRAPH FOR THIS STIMULIS 
  if(GRAPH_SAVE == TRUE) {
     ggsave(plot = p, path="figs/level_block1/FIG_B1/S1_3", filename =paste0("FULL_STUDY_STACK_",s,".png"), units = c("in"), width = 8, height = 24,  bg='#ffffff'  )}
...
```{r block1_sds_Study3fake, message=FALSE, warning = FALSE}
...
## geom_density_ridges with stat density
```{r}

  
  # ( b <- ggplot(df, aes(x = value, y = Study, fill = Sample, color = Sample, height = after_stat(density))) +
  #   geom_density_ridges2(stat = "density", scale = 1.5, panel_scaling = TRUE, rel_min_height = 0.01, alpha = 0.25) +
