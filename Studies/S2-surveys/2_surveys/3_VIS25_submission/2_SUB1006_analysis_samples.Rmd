...
ref_s4_sd_questions <-  c("DESIGN","DATA","POLITICS", "ALIGN","INTENT","TRUST","BEAUTY")  

ref_labels <- readRDS("data/input/REFERENCE/ref_labels.rds")
ref_labels_min <- readRDS("data/input/REFERENCE/ref_labels_S4.rds")
...
df_b1_sd_questions_long_S4 <- df_sd_questions_long_S4 %>% 
  filter(Assigned.Block==1) %>% 
  filter(TIME =="PRE") %>% select(-TIME) %>% droplevels()

df_b1_sd_questions_long <- rbind(df_b1_sd_questions_long, df_b1_sd_questions_long_S4) %>% 
  mutate(
...
questions <- ref_s4_sd_questions #has qs in right order
...
### S1_S3 studies stacked density ridges
...
df <- df %>% filter(Study!="Study2") %>% droplevels()
