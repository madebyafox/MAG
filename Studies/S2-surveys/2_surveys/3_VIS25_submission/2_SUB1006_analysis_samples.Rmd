...
## !! BLOCK 1 STIMULUS BY SAMPLE (vertical facet) !!

##### study 1-3 stacked density ridges
```{r plot_ridglines_sds_block1_STIMULUS}
  
#### DENSITY RIDGES#############################################################################
#### loop over questions and stimuli, vertically stack studies, color by sample


## DEFINE STIMULI & QUESTIONS
stimuli <- ref_stimuli %>% filter( str_detect(ID, "B0|B1")) %>% 
  mutate(STIMULUS = factor(ID)) %>% pull(STIMULUS) 
stimuli <- as.vector(stimuli)
questions <- ref_min_sd_questions

## SETUP DATAFRAME
df <- df_sd_questions_long %>% filter(Assigned.Block==1) %>% 
          filter(QUESTION %in% ref_min_sd_questions) %>% 
          select(PID, Assigned.Block, Sample, Study, STIMULUS, QUESTION, value) %>% 
          mutate(QUESTION = factor(QUESTION, levels=ref_min_sd_questions)) %>% #set q order
          droplevels() 
...
for (s in stimuli){
  ...
  if(GRAPH_SAVE == TRUE) {
     ggsave(plot = p, path="figs/level_block1", filename =paste0("STUDY_STACK_",s,".png"), units = c("in"), width = 8, height = 24,  bg='#ffffff'  )} 
...
##### study 1-4 stacked density ridges
```{r plot_ridglines_sds_block1_STIMULUS}
...
stimuli <- ref_stimuli %>% filter( str_detect(ID, "B0|B1")) %>% 
  mutate(STIMULUS = factor(ID)) %>% pull(STIMULUS) 
stimuli <- as.vector(stimuli)
questions <- ref_s4_sd_questions

## SETUP DATAFRAME

df <- df_b1_sd_questions_long %>% 
          select(PID, Assigned.Block, Sample, Study, STIMULUS, QUESTION, value)  %>% 
          mutate(QUESTION = factor(QUESTION, levels=ref_s4_sd_questions)) %>% #set q order
          droplevels() 
...
for (s in stimuli){
  ...
  ## SAVE GRAPH FOR THIS STIMULIS 
  if(GRAPH_SAVE == TRUE) {
     ggsave(plot = p, path="figs/level_block1", filename =paste0("FULL_STUDY_STACK_",s,".png"), units = c("in"), width = 8, height = 24,  bg='#ffffff'  )} 
