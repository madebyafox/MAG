...
### [3] Sankey Categorical Variables Change (Study 3)
...
ds <- df %>% 
  ...
  mutate(
    node = factor(node, levels=order_maker),
    next_node= factor(next_node, levels=order_maker),
...
if(GRAPH_SAVE == TRUE) {
    ggsave(plot = S, path="figs/MVP/FIG_Descriptives", filename =paste0("S3_ID_CHANGE_by_STIMLUS.png"), units = c("in"), width = 16, height = 8,  bg='#ffffff'  )
  }
```
...
ds <- df %>% 
  ...
  mutate(
    node = factor(node, levels=order_age),
    next_node= factor(next_node,order_age),
    match = ifelse(node==next_node, 1, 0.5), # try to highlight throughflows
  ) 
...
if(GRAPH_SAVE == TRUE) {
    ggsave(plot = S, path="figs/MVP/FIG_Descriptives", filename =paste0("S3_AGE_CHANGE_by_STIMLUS.png"), units = c("in"), width = 16, height = 8,  bg='#ffffff'  )
  }
...
if(GRAPH_SAVE == TRUE) {
    ggsave(plot = S, path="figs/MVP/FIG_Descriptives", filename =paste0("S3_GENDER_CHANGE_by_STIMLUS.png"), units = c("in"), width = 16, height = 8,  bg='#ffffff'  )
  }
...
if(GRAPH_SAVE == TRUE) {
    ggsave(plot = S, path="figs/MVP/FIG_Descriptives", filename =paste0("S3_ENCOUNTER_CHANGE_by_STIMLUS.png"), units = c("in"), width = 16, height = 8,  bg='#ffffff'  )
  }
