...
ggplot(df_graphs, aes(x=STIMULUS, y=MAKER_LATENCY, color=Assigned.Block)) +
...
ggplot(df_graphs, aes(x=STIMULUS, y=CHART_LATENCY, color=Assigned.Block)) +
  geom_boxplot(position=position_dodge(0.9))+
  geom_jitter(position=position_jitterdodge(), alpha = 0.3) + 
...
# EDUCATION by SAMPLE
ggplot(data = df_data, aes( x = D_education, fill = Assigned.Block )) + 
...
# AGE by SAMPLE
ggplot(data = df_data, aes( x = D_age, fill = Assigned.Block )) + 
  geom_bar(position = position_dodge()) +
...
# POLITICAL PARTY 
ggplot(data = df_data, aes( x = D_politicalParty, fill = Assigned.Block )) + 
  geom_bar(position = position_dodge()) +
...
g <- ggplot(df_data, aes(x=Assigned.Block, y=D_politicsSocial, color=Assigned.Block)) +
...
leftside <- rep("left-leaning",6)
rightside <- rep("right-leaning",6)
g <- ggplot(df_data, aes(x=Assigned.Block, y=D_politicsFiscal, color=Assigned.Block)) +
  geom_boxplot(position=position_dodge(0.9), width = 0.5)+
  geom_jitter(position=position_jitterdodge(), alpha = 0.3) +
...
ggplot(data = df_data, aes( x = D_politicsFiscal, y= D_politicsSocial, color = Assigned.Block)) +
  geom_point(alpha = 0.5) + 
  geom_smooth(method="lm", se=TRUE, aes(fill= Assigned.Block)) + 
  facet_wrap(~ Assigned.Block) + 
  theme_minimal() + 
  easy_add_legend_title("") + 
...
#PLATFORM CHOICE
ggplot( df_data, aes( x = PLATFORM, fill = Assigned.Block)) + 
...
## TODO Explore Demographics of Excluded
