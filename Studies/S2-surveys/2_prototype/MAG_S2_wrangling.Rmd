...
# 1 row per subject 
# block 1 data collar 
df_raw <- read_csv("data/CLEAN_MAG_S2_PROLIFIC-DATACOLLAR_1.csv", col_names = TRUE)


#### MASTER WIDE FORMAT DATA FRAME [1 row / qualtrics submission] ################
# drop first two rows (qualtrics_specs)
df_master <- df_raw[-c(1:2),] %>% 
  select(
  ...
  select ( #reodering
    ...
    StartDate, duration.sec, duration.min, 
    ORDER, PLATFORM,
    P_BROWSER_Browser, P_BROWSER_OS, T_BROWSER_Browser, T_BROWSER_OS, 
    D_gender:D_age, SCREEN_workMethod:SCREEN_socialMedia_TEXT, 
    PURPOSE, FEEDBACK , 
