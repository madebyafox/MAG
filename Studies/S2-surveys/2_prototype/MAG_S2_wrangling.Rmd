...
df_exclude <- df_data %>% 
  ...
  select(
    ID.Qualtrics, ID.Prolific, ID.Study, Assigned.Block, Source, Progress, 
...
table(df_exclude$Assigned.Block)
...
##TODO FIX THIS AFTER REAL BLOCK 1 DATA IS COLLECTED 
...
s = "B0-0"
df <- df_sd_questions %>% select(1:5, s) %>% filter(!is.na(s)) %>%  mutate(
  value = `B0-0`
)
