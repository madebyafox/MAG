...
df <- df_graphs %>%
  ...
  filter(STIMULUS != "B0-0") %>% 
  select(PID, STIMULUS, STIMULUS_CATEGORY, BLOCK, ENCOUNTER, CHART_LIKE, CHART_TRUST, CHART_BEAUTY, CHART_INTENT) %>% 
  mutate(
...
mm.BrP <- lmer(CHART_TRUST ~ CHART_BEAUTY + (1|PID), data = df)
# :: TEST fixed factor 
...
print("A model with BEAUTY IS a better fit than (random effect) participant alone")
