...
# df <- df_graphs %>% select(all_of(all_q))
# # plot_data <- list(df_graphs$MAKER_CONF, df_graphs$AGE_CONF, df_graphs$GENDER_CONF, df_graphs$TOOL_CONF)
# 
# dat <- data.frame(
#   VARIABLE = all_q,
#   DISTRIBUTION=""
...
# 
# tt(dat) %>% 
#   plot_tt(j=2, fun = "histogram", data = df) %>% 
#   plot_tt(j=2, fun = "density", data = df) 
#   # plot_tt(j=4, fun = "bar", data = plot_data) 
...
# 
# 
# ############ WORKING SPARKLINE CATEGORICAL STACKED BAR 
# #####################
# 
# 
...
# 
#     # scale_fill_manual(values = my_palettes(name=as.char(pal), direction = "1"), name = "",  guide = guide_legend(reverse = TRUE)) +
#     coord_flip() + theme_void() + 
#     easy_remove_axes() + easy_remove_legend()
# }
# 
# 
# ### CREATE CATEGORICAL DF LIST FOR TT
...
# 
# tab <- data.frame(
#   VARIABLE = ref_cat_questions,
#   DISTRIBUTION = ""
# )
# 
# 
# tt(tab) %>%
#   plot_tt(j=2, fun=g, data = df_cat, height = 3)
...
#### SETUP TABLE
tab <- data.frame(
  VARIABLE = c(ref_cat_questions, ref_conf_questions, ref_sd_questions),
  DISTRIBUTION = "",
  STATISTICS = c("","","",stat)
)
...
t <- tt(tab, theme = "void") %>% 
  ...
  plot_tt(j=2, i= 4:18, fun="density", data = df_num, color="darkgrey") %>%
  
  style_tt(j=2, align="c", monospace = TRUE) 

t
save_tt(t, output="figs/tables/sparklines.png")
save_tt(t, output="figs/tables/sparklines.tex")
```
...
print("SD OF QUESTION CONFIDENCE")
(q_conf <- df_conf_long %>% 
  group_by(QUESTION) %>% 
  summarize(
    m = mean(value),
...
print("SD AND RANGES OF PARTICIPANT CONFIDENCE SCORES")
...
First looking at variance in confidence by question, we see that the proportion of participants with less than 10 in confidence scores (less than 20 points difference in range), is 21% for AGE, 21% for GENDER, 15% for MAKER and 22% for TOOL 


Next, looking at variance in confidence by participant, we see that only 4 of the 218 participants (2%) had less than 20 point 
