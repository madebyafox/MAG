...
f <-  "ACTION ~ ?? (1|PID)"
mm <- glmer(CHART_ACTION2 ~ LIKE_Z + STIMULUS_CATEGORY * INTENT_Z + (1|PID), 
...
## PLOT PRED
plot_model(m, type = "pred", terms = c("STIMULUS_CATEGORY", "LIKE_Z", "INTENT_Z")) + 
  labs(subtitle = f) + theme_minimal()
...
mm <- glmer(ENCOUNTER ~ LIKE_Z + STIMULUS_CATEGORY + MAKER_ID + (1|PID), 
                data = df,family = "binomial",
             control=glmerControl(optimizer="bobyqa", #would not converge under Nelder)Mead
               optCtrl=list(maxfun=2e5)))
car::Anova(mm, type = 2)
