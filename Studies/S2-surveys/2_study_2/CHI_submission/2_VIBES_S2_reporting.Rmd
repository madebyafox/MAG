...
(n = `r nrow(df_participants)` ) survey respondents answered questions about some subset of the stimuli, (common stimulus B0-0 and 4 additional images defined as a block), yielding (o = `r nrow(df_full)`) stimulus-level observations.
...
df <- df_full %>% select(Distribution, MAKER_CONF, AGE_CONF, GENDER_CONF, TOOL_CONF) %>% 
  pivot_longer(
...
df %>% 
  ggplot(aes(x=CONFIDENCE, y=QUESTION)) + 
  geom_density_ridges(scale = 0.75, quantile_lines = TRUE, alpha = 0.75) + 
  scale_x_continuous(limits = c(0,100))+
  theme_minimal() + 
  labs(title = "Confidence by Survey Question")
...
#################### ALL QUESTIONS across ALL STIMULUS #####################
  ...
  #RAINCLOUD PLOT
  R <- ggplot(df, aes(y = fct_rev(QUESTION), x = value)) +
  ...
  ggsave(plot = R, path="figs/_master", filename =paste0("combined_stimuli","_rain.png"), units = c("in"), width = 10, height = 14  )
 ...
 x <-  ggplot(df, aes(x = value, y = fct_rev(QUESTION), fill = fct_rev(QUESTION))) +
    geom_density_ridges(scale = 1,quantile_lines = TRUE, alpha = 0.75) + 
...
df %>% ggplot(aes(y = fct_rev(MAKER_ID), x= MAKER_POLITIC, fill = fct_rev(MAKER_ID))) + 
...
df %>% ggplot(aes(y = fct_rev(MAKER_ID), x= MAKER_DATA, fill = fct_rev(MAKER_ID))) + 
  geom_density_ridges(scale = 0.55,quantile_lines = TRUE, alpha = 0.75) + 
  stat_dotsinterval(side = "bottom", scale = 0.7, slab_linewidth = NA, point_interval = "mean_qi") +
  stat_summary(fun=mean, geom="text", colour="blue",  fontface = "bold", 
               vjust=+2, hjust = 0, aes( label=round(..x.., digits=0)))+
...
df %>% ggplot(aes(y = fct_rev(MAKER_ID), x= MAKER_DESIGN, fill = fct_rev(MAKER_ID))) + 
  geom_density_ridges(scale = 0.55,quantile_lines = TRUE, alpha = 0.75) + 
  stat_dotsinterval(side = "bottom", scale = 0.7, slab_linewidth = NA, point_interval = "mean_qi") +
  stat_summary(fun=mean, geom="text", colour="blue",  fontface = "bold", 
               vjust=+2, hjust = 0, aes( label=round(..x.., digits=0)))+
...
#### WIP exploring correlation matrices
```{r}
...
ggcorr(df, 
      ...
      layout.exp = 2, hjust = 0.8, size = 3) +
      geom_point(size = 10, aes(color = coefficient > 0, alpha = abs(coefficient) > 0.35)) +
      scale_alpha_manual(values = c("TRUE" = 0.25, "FALSE" = 0)) +
      guides(color = FALSE, alpha = FALSE)
```


#### correlation matrices by groups
```{r}
...
(x <-   ggscatmat(df, columns = 1:11, color = "ENCOUNTER", alpha = 0.8) + 
    scale_color_manual(values = my_palettes(name="traffic", direction = "1"))+
    theme_minimal())
