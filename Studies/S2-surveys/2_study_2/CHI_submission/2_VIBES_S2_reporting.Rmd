...
df_b2 <- df_graphs %>% 
  ...
  mutate(MAKER_ID = fct_rev(MAKER_ID)) # reorder factors)
...
Participants were asked: **Who do you think is most likely responsible for having this image created?**
...
S <- ggplot(ds, aes(x = x, 
  ...
  geom_alluvial(width = 0.25, flow.alpha = 0.5, node.alpha = 1) +
...
I <- ggplot(df, aes(STIMULUS,MAKER_CONF)) + 
  ...
  scale_color_manual(values = my_palettes(name="blackred", type = "discrete")) +
...
*To explore whether variance in **confidence** in maker identification, we visualize the distribution of confidence scores for each stimulus, where connecting lines indicate responses for a single participant. Participants with less than 20 point difference between their minimum and maximum confidence scores are indicated in red, ( `r`* `p_lowvar` of the `r nrow(df_b2_p)` participants.) Note that the participants with low variance in confidence scores are clustered near the high end of the confidence. The mean confidence score for each stimulus is greater than 50%, and no participants indicated consistently low confidence across all stimuli, indicating that in general, participants were able to respond thoughtfully to the maker identification question.
...
#MAKER AGE BY STIMULUS
## GGSTATSPLOT
ggbarstats( data = df, x = "MAKER_AGE", y = "STIMULUS",
...
# MAKER_AGE & CONFIDENCE
b <- ggplot(df, aes( x = (STIMULUS), y = AGE_CONF, color = MAKER_AGE)) + 
...
# AGE_CONFIDENCE 
c <- ggplot(df, aes( x = (STIMULUS), y = AGE_CONF)) + 
  geom_boxplot(position=position_dodge(0.9), width = 0.5)+
  geom_jitter(width = 0.10, alpha = 0.4) +
  labs (title = "") +
  labs (x = "") + 
