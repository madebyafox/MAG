...
_Across all participants and all stimuli, are these questions answered with the same degree of confidence?_
...
#################### ALL QUESTIONS across ALL STIMULUS #############################################################
  ...
  ggsave(plot = g, path="figs/_master", filename =paste0("combined_stimuli","_ggdist.png"), units = c("in"), width = 10, height = 14  )
  ...
  c <-ggplot(d, aes(x = value, y = fct_rev(QUESTION), fill=STIMULUS_CATEGORY))+ 
    ...
    ggsave(plot = c, path="figs/_master", filename =paste0("combined_by_category","_ridges.png"), units = c("in"), width = 10, height = 14  )
...
#################### ALL QUESTIONS across ALL STIMULUS #############################################################
  ...
  ggsave(plot = plot_master_questions, path="figs/_master", filename =paste0("ABS_combined_stimuli","_box.png"), units = c("in"), width = 10, height = 14  )
  ...
  ggsave(plot = g, path="figs/_master", filename =paste0("ABS_combined_stimuli","_ggdist.png"), units = c("in"), width = 10, height = 14  )
  ...
  x <-ggplot(d, aes(x = value, y = fct_rev(QUESTION), fill = category)) +
    ...
    ggsave(plot = x, path="figs/_master", filename =paste0("ABS_combined_stimuli","_ridges.png"), units = c("in"), width = 10, height = 14  )
  ...
  c <-ggplot(d, aes(x = value, y = fct_rev(QUESTION), fill=STIMULUS_CATEGORY))+ 
    ...
    ggsave(plot = c, path="figs/_master", filename =paste0("ABS_combined_by_category","_ridges.png"), units = c("in"), width = 10, height = 14  )
...
d <- df_graphs %>% 
  # CENTER SCORES AT 0 RATHER THAN 50
  # (range of values on slider was 0 <-> 100)
  mutate(
    MAKER_DESIGN = MAKER_DESIGN - 50,
    MAKER_DATA = MAKER_DATA - 50,
    MAKER_POLITIC = MAKER_POLITIC - 50,
    MAKER_ARGUE = MAKER_ARGUE - 50,
    MAKER_SELF = MAKER_SELF - 50,
    MAKER_ALIGN = MAKER_ALIGN - 50,
    MAKER_TRUST = MAKER_TRUST - 50,
    CHART_LIKE = CHART_LIKE - 50,
    CHART_BEAUTY = CHART_BEAUTY - 50,
    CHART_INTENT = CHART_INTENT - 50,
    CHART_TRUST = CHART_TRUST - 50,
  ) %>% 
  mutate(
    abs_MAKER_DESIGN = abs(MAKER_DESIGN),
    abs_MAKER_DATA = abs(MAKER_DATA),
    abs_MAKER_POLITIC = abs(MAKER_POLITIC),
    abs_MAKER_ARGUE = abs(MAKER_ARGUE),
    abs_MAKER_SELF = abs(MAKER_SELF),
    abs_MAKER_ALIGN = abs(MAKER_ALIGN),
    abs_MAKER_TRUST = abs(MAKER_TRUST),
    abs_CHART_LIKE = abs(CHART_LIKE),
    abs_CHART_BEAUTY = abs(CHART_BEAUTY),
    abs_CHART_INTENT = abs(CHART_INTENT),
    abs_CHART_TRUST = abs(CHART_TRUST)
  )
...
# print("PARTIAL CORRELATION NO RANDOM EFFECT")
...
df <- df_graphs %>% select(MAKER_DESIGN, MAKER_DATA, MAKER_POLITIC, MAKER_ARGUE,
                    MAKER_SELF, MAKER_ALIGN, MAKER_TRUST, CHART_LIKE, CHART_BEAUTY, 
                    CHART_INTENT, CHART_TRUST, 
...
(x <-   ggscatmat(df, columns = 1:11, color = "MAKER_ID", alpha = 0.8) + 
    ...
    theme_minimal())
ggsave(plot = x, path="figs/corr_by_group", filename =paste0("maker_id_corr_x.png"), units = c("in"), width = 14, height = 10 )
...
if(graph_render){
  ...
  (x <-   ggscatmat(df, columns = 1:11, color = "MAKER_AGE", alpha = 0.8) + 
    ...
    theme_minimal())
  ggsave(plot = x, path="figs/corr_by_group", filename =paste0("maker_age_corr_x.png"), units = c("in"), width = 14, height = 10 )
}
```
...
if(graph_render){
  ...
  (x <-   ggscatmat(df, columns = 1:11, color = "MAKER_GENDER", alpha = 0.8) + 
    ...
    theme_minimal() )
  ggsave(plot = x, path="figs/corr_by_group", filename =paste0("maker_gender_corr_x.png"), units = c("in"), width = 14, height = 10 )
}
```
**Interesting patterns to explore further**
...
df <- df_tools %>% select(MAKER_DESIGN, MAKER_DATA, MAKER_POLITIC, MAKER_ARGUE,
                    MAKER_SELF, MAKER_ALIGN, MAKER_TRUST, CHART_LIKE, CHART_BEAUTY, 
                    CHART_INTENT, CHART_TRUST, 
                    PID, STIMULUS, BLOCK, STIMULUS_CATEGORY, 
                    TOOL_ID) 
 ...
 (x <-  ggscatmat(df, columns = 1:11, color = "TOOL_ID", alpha = 0.8) + 
    ...
    theme_minimal() )
  ggsave(plot = x, path="figs/corr_by_group", filename =paste0("tool_id_corr_x.png"), units = c("in"), width = 14, height = 10 )
}  
```
...
(x <-   ggscatmat(df, columns = 1:11, color = "ENCOUNTER", alpha = 0.8) + 
    ...
    theme_minimal())
  ggsave(plot = x, path="figs/corr_by_group", filename =paste0("encounter_corr_x.png"), units = c("in"), width = 14, height = 10 )
...
df <- df_actions %>% select(MAKER_DESIGN, MAKER_DATA, MAKER_POLITIC, MAKER_ARGUE,
                    MAKER_SELF, MAKER_ALIGN, MAKER_TRUST, CHART_LIKE, CHART_BEAUTY, 
                    CHART_INTENT, CHART_TRUST, 
                    PID, STIMULUS, BLOCK, STIMULUS_CATEGORY, 
                    CHART_ACTION) 
...
(x <- ggscatmat(df, columns = 1:11, color = "CHART_ACTION", alpha = 0.2) + 
    ...
    theme_minimal() )
ggsave(plot = x, path="figs/corr_by_group", filename =paste0("chart_action_corr_x.png"), units = c("in"), width = 14, height = 10  )
...
## CREATE subject.id for within comparisons
df <- df_graphs %>% mutate( subject.id = PID)

### REPEATED MEASURES ANOVA?

# print("Repeated Measures ANOVA")
# a <-aov (MAKER_DATA ~ MAKER_ID + Error(PID), data=df)
# summary(a)
# report(a)
...
```{r}
# DESIGN BY DATA
df %>% ggplot(aes(x = MAKER_DATA, y = MAKER_DESIGN, color = MAKER_ID)) +
  geom_point() +
  scale_color_manual(values = my_palettes(name="reds", direction = "1"), name = "",  guide = guide_legend(reverse = TRUE)) +   
  facet_grid(MAKER_ID ~ .) + 
  geom_line(stat="lm")+
  theme_minimal()
...
grouped_ggscatterstats(
  ...
  type             = "parametric",
  # bf.message = FALSE,
  grouping.var     = MAKER_ID,
  point.args = list(size = 1, alpha = 0.4, stroke = 0),
