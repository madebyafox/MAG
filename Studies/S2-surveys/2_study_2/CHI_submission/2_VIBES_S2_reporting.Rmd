...
*A total of `r n_tumblr + n_prolific` participants were recruited from US-located English speaking users of Tumblr (n = `r n_tumblr`) and Prolific (n = `r n_prolific`).  
...
df_b2 <- df_data %>% filter(Assigned.Block ==2)

```

(N = `r nrow(df_b2)` ) survey respondents answered questions about stimulus block 2. 
...
#MAKER IDENTIFICATION BY STIMULUS
## GGSTATSPLOT
ggbarstats( data = df, x = "MAKER_ID", y = "STIMULUS",
...
b <- ggplot(df, aes( x = (STIMULUS), y = MAKER_CONF, color = MAKER_ID)) + 
...
# MAKER_CONFIDENCE 
c <- ggplot(df, aes( x = (STIMULUS), y = MAKER_CONF)) + 
  geom_boxplot(position=position_dodge(0.9), width = 0.5)+
...
ie. mixed model, but also look at stability of individual choice across stimuli 
maybe treat individual as fixed factor and look at amount of variance explained?
