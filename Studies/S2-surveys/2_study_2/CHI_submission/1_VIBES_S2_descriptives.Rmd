...
ggplot(df_data, aes(x=fct_rev(Assigned.Block), y=duration.min, color=Assigned.Block))+
  ...
  labs( y = "Survey Response Time (mins)", x="",
        ...
        subtitle = "(distributions of response times are similar across both SAMPLES and stimulus BLOCKS, as expected)") +
...
ggplot(df, aes(x = duration.min, y = fct_rev(Assigned.Block), fill = fct_rev(Assigned.Block))) +
  ...
  theme(legend.position = "none") +
    labs( x = "Survey Response Time (mins)", y="",
        ...
        caption = "(As expected, distributions of response times are similar across SAMPLES and stimulus BLOCKS)") 
...
# OVERALL AGE by SAMPLE
ggstatsplot::ggbarstats(df, x= D_age, y=Distribution,
                          label = "both", 
...
# GGSTATSPLOT BAR CHART
# OVERALL GENDER by SAMPLE
ggstatsplot::ggbarstats(df, x= D_gender, y=Distribution, 
                        results.subtitle = FALSE,
                        legend.title = "Gender", label = "both")  +
    scale_fill_viridis(discrete=TRUE, option="viridis", direction=-1) + 
    labs (title = "Participant GENDER by SAMPLE",
          caption = "As expected, greater variance in GENDER in Tumblr sample") + 
    theme_minimal() 


# GGSTATSPLOT BAR CHART
# OVERALL GENDER by SAMPLE (collapsed)
ggstatsplot::ggbarstats(df, x= D_gender_collapsed, y=Distribution,
                        results.subtitle = FALSE, 
                        legend.title = "Other*",
                         label = "both") +
    scale_fill_viridis(discrete=TRUE, option="viridis", direction=-1) + 
    labs (title = "Participant GENDER by SAMPLE", 
          caption = "*OTHER includes: nonbinary, prefer not to say or self-describe") +
    theme_minimal() 
...
# GGSTATSPLOT BAR CHART
# OVERALL EDUCATION by SAMPLE
ggstatsplot::ggbarstats(df, x= D_education, y=Distribution, 
                        results.subtitle = FALSE,
                        legend.title = "Education") +
    scale_fill_viridis(discrete=TRUE, option="viridis", direction=-1) + 
    labs (title = "Participant EDUCATION by SAMPLE",
          caption = "Similar distribution of education categories across Prolific and Tumblr samples") + 
    theme_minimal() 
...
# GGSTATSPLOT BAR CHART
# COLLAPSED EDUCATION by SAMPLE
ggstatsplot::ggbarstats(df, x= D_education_collapsed, y=Distribution, 
                        results.subtitle = FALSE,
                        legend.title = "Education",
                        label = "both") +
    scale_fill_viridis(discrete=TRUE, option="viridis", direction=-1) + 
    labs (title = "Participant EDUCATION (collapsed) by SAMPLE",
          caption = "Smaller Tumblr sampler skews slighly toward higher levels of education") + 
    theme_minimal() 
...
# GGSTATSPLOT BAR CHART
# POLITICAL AFFILIATION by Sample
ggstatsplot::ggbarstats(df, x= D_politicalParty, y=Distribution, 
                        results.subtitle = FALSE,
                        legend.title = "") +
    scale_fill_viridis(discrete=TRUE, option="C", direction=1) + 
    labs (title = "Political Affiliation by SAMPLE",
          caption = "The Tumblr sample is more left-leaning than the larger Prolific sample. Overall the sample has few right-leaning individuals.") + 
    theme_minimal() 
