  ...
  mutate(
    ...
    D_age = factor(D_age, 
                   levels = c("25-34 years old" ,
                              ...
                              "55-64 years old" ,
                              "65+ years old"   ), 
                   labels = c("25-34", "18-24", "45-54","35-44","55-64","65+ years"))
  ) %>%
  rename_at(
...
df_data <- df_data %>% 
  filter(Scope != "pilot") %>% 
  mutate(ID.Study = droplevels(ID.Study))
...
df_questions <- df_data %>% 
  select(
  ID.Qualtrics:ID.Study, Assigned.Block, PLATFORM, 
  contains("_Q_"), contains("loop")
) %>% 
  pivot_longer( #PIVOT ON stimulus
  cols = contains("_Q_"),
...
Participant response times ranged from `r min(df_data$duration.min)` to `r max(df_data$duration.min)` minutes, with a mean response time of `r mean(df_data$duration.min)` minutes.
...
ggplot(df, aes(x=fct_rev(Assigned.Block), y=MAKER_LATENCY, color=Assigned.Block))+
  geom_boxplot(position=position_dodge(0.9))+
  geom_jitter(position=position_jitterdodge(jitter.width=1.5), alpha = 0.3) +
  coord_flip() + 
  labs( y = "Maker Page Response Time (mins)", x="",
        title = "Maker Page Response Time by Sample",
        subtitle = "(expect similiar across samples)") +
  theme_minimal() + theme(legend.position = "none") 
...
ggplot(df, aes(x = MAKER_LATENCY, y = fct_rev(Assigned.Block), fill = fct_rev(Assigned.Block))) +
  geom_density_ridges(scale=0.8) +
  # geom_boxplot()+
  stat_pointinterval()+
  # theme_ridges() +
...
```
Maker page response times ranged from `r min(df_graphs$MAKER_LATENCY)` to `r max(df_graphs$MAKER_LATENCY)` minutes, with a mean response time of `r round(mean(df_graphs$MAKER_LATENCY),1)` minutes.
...
## BOXPLOT — CHART RESPONSE TIME  
ggplot(df, aes(x=fct_rev(Assigned.Block), y=CHART_LATENCY, color=Assigned.Block))+
  geom_boxplot(position=position_dodge(0.9))+
  geom_jitter(position=position_jitterdodge(jitter.width=1.5), alpha = 0.3) +
  coord_flip() + 
  labs( y = "Chart Page Response Time (mins)", x="",
        title = "Chart Page Response Time by Sample",
        subtitle = "(expect similiar across samples)") +
  theme_minimal() + theme(legend.position = "none") 
...
## RIDGEPLOT — Chart PAGE RESPONSE
ggplot(df, aes(x = CHART_LATENCY, y = fct_rev(Assigned.Block), fill = fct_rev(Assigned.Block))) +
  geom_density_ridges(scale=0.8) +
  # geom_boxplot()+
  stat_pointinterval()+
  # theme_ridges() +
...
Chart page response times ranged from `r min(df_graphs$CHART_LATENCY)` to `r max(df_graphs$CHART_LATENCY)` minutes, with a mean response time of `r round(mean(df_graphs$CHART_LATENCY),1)` minutes.
...
```
Across all six samples (n=40/sample) participants tended to report center-left leaning social values (M = `r round(mean(df_data$D_politicsSocial),2)`, SD = (`r round(sd(df_data$D_politicsSocial),2)` on a [0,100] scale);  with slightly more central fiscal values (M = `r round(mean(df_data$D_politicsFiscal),2)`, SD = (`r round(sd(df_data$D_politicsFiscal),2)`. Overall, `r l_politics*100`% of respondents identify with left-leaning social values (vs) `r r_politics*100` identifying as right-leaning; while `r round(l_fiscal*100,2)`% of respondents reported left-leaning fiscal values (vs) `r round(r_fiscal*100,2)` identify as right-leaning. 
...
#   geom_sf(data = df_data$D_zipcode, aes(fill = AREA), linewidth = 0.1) +
...
# ggplot() +
#   geom_sf(data = ca) +
#   geom_sf(aes(fill = ALAND10, color = ALAND10), data = df_geo) +
#   theme_void()
# 
# 
# 
...
# 
# 
# #get ALL the zipcodes and their zip code tabulation areas (zctas)
# zipcodes <- zctas()
# 
# #join zip code geospatial data to subject-level dataframe                                  
# df_geo <- left_join(df_data, master_zips, join_by(D_zipcode==ZCTA5CE20))    
# 
# 
# #reshape data 
# g <- df_geo %>% group_by(D_zipcode) %>% summarise(
#   zip = D_zipcode, 
#   n = n()
# )
# 
# ggplot(df_geo) +
#   geom_sf(aes(fill = D_zipcode))
# 
# 
# ##INSTALL CHLOROPLETHR
# # https://ari-lamsteins-courses.thinkific.com/courses/take/mapmaking-in-r-with-choroplethr/lessons/204724-zip-code-choropleths-part-2
...
# BEHAVIOUR CHOICE
ggplot (df, aes ( x = ENCOUNTER, fill = Assigned.Block)) + 
...
a <- ggplot (df, aes( x = MAKER_ID, fill = Assigned.Block)) + 
  geom_bar(position = "dodge") +
...
b <- ggplot(df, aes( x = MAKER_ID, y = MAKER_CONF, color = Assigned.Block)) + 
  # geom_boxplot(position=position_dodge(0.9))+
  geom_jitter(position=position_jitterdodge(), alpha = 0.3) + 
...
a <- ggplot (df, aes( x = MAKER_AGE, fill = Assigned.Block)) + 
  geom_bar(position = "dodge") +
  facet_grid( df$ENCOUNTER) + 
  labs (x = "") + 
  theme_minimal() +
...
# MAKER_CONFIDENCE by IDENTIFICATION  
b <- ggplot(df, aes( x = MAKER_AGE, y = MAKER_CONF, color = Assigned.Block)) + 
  # geom_boxplot(position=position_dodge(0.9))+
  geom_jitter(position=position_jitterdodge(), alpha = 0.3) + 
...
for (s in stimuli){
  ...
  plots_stimulus[[i]] <- x
  ggsave(plot = x, path="figs/plots_stimulus", filename =paste0(s,".png"), units = c("in"), width = 10, height = 14  )
}
...
for (c in categories){
  ...
  plots_category[[i]] <- x
  ggsave(plot = x, path="figs/plots_category", filename =paste0(c,".png"), units = c("in"), width = 10, height = 26)
...
for (q in questions){
  ...
  ggsave(plot = x, path="figs/plots_s_question", filename =paste0(q,".png"), units = c("in"), width = 10, height = 26)
}
...
for (q in questions){
  ...
  ggsave(plot = x, path="figs/plots_c_question", filename =paste0(q,".png"))
}
