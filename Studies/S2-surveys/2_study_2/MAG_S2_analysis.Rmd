...
## IMPORTANT 
graph_render = FALSE #set to true to generate all the SD graphs and save to folders 
...
Participant response times ranged from `r desc.duration$min` to `r desc.duration$max` minutes, with a mean response time of `r round(desc.duration$mean,2)` minutes, SD = `r (round(desc.duration$sd,2))`.

### Maker Page Response Time
```{r demo-maker-response-time}
...
# OVERALL AGE
ggstatsplot::ggbarstats(df_data, x= D_age, y=Scope) +
    theme_minimal() +
    scale_fill_viridis(discrete=TRUE, option="viridis", direction=-1) + 
...
# OVERALL AGE
ggstatsplot::ggbarstats(df_data, x= D_gender, y=Scope) +
    theme_minimal() +
    scale_fill_viridis(discrete=TRUE, option="viridis", direction=-1) + 
...
desc.gender <- table(df_data$D_gender) %>% prop.table() 
names(desc.gender) <- levels(df_data$D_gender)
n_participants <- nrow(df_data)


```


`r n_participants` individuals from Prolific participated in Study 2, ( `r round(desc.gender[["Female"]],2)*100`% Female, `r round(desc.gender[["Male"]],2)*100`% Male, `r round(desc.gender[["Non-binary / third gender"]],2)*100`% Non-binary, `r (round((desc.gender[["Prefer not to say"]] + desc.gender[["Prefer to self-describe"]]),2)) * 100`% Other).
...
Across all six samples (n=40/sample) participants tended to report center-left leaning social values (M = `r round(mean(df_data$D_politicsSocial),2)`, SD = (`r round(sd(df_data$D_politicsSocial),2)`) on a [0,100] scale);  with slightly more central fiscal values (M = `r round(mean(df_data$D_politicsFiscal),2)`), SD = (`r round(sd(df_data$D_politicsFiscal),2)`). 

Overall, `r l_politics*100`% of respondents identify with left-leaning social values (vs) `r r_politics*100`% identifying as right-leaning; while `r round(l_fiscal*100,2)`% of respondents reported left-leaning fiscal values (vs) `r round(r_fiscal*100,2)`% identify as right-leaning. 
...
df <- df_graphs_coded %>% select( ID.Prolific, STIMULUS, BLOCK, CATEGORY, MAKER_ID, MAKER_DETAIL, CODE_M_ID_SPECIFIC) %>% 
  separate_longer_delim(
...
ggplot(df,  aes(x = MAKER_ID, fill = CODED_MAKER )) + 
  geom_bar(position = "stack") +
  facet_grid( rows = vars(BLOCK), cols = vars(fct_rev(CATEGORY))) + 
  theme_minimal() + 
...
ggplot(df,  aes(x = CODED_MAKER, fill = STIM, )) + 
  geom_bar(position = "stack") + 
  coord_flip() + 
  theme_minimal() 
