...
df_raw_tumblr <- read_csv("data/input/CLEAN_MAG_S2_TUMBLR_PAID_ALL.csv", col_names = TRUE)
...
df_raw_tumblr <- df_raw_tumblr[-c(1:2),]
...
df_raw_tumblr <- df_raw_tumblr %>% 
  select( - t_example_question1, -t_example_question2,
          -contains("Straightlining")) %>% 
  mutate(
...
#DROP WIP DATAFRAMES 
rm(df_raw_datacollar, df_raw_bluecollar, df_raw_b1, df_raw_b2, df_raw_b3, df_raw_b4, df_raw_b5, df_raw_b6, df_raw_pilot, df_raw_prolific, df_raw_tumblr)
...
print("Number of successful surveys by distribution")
table(df_data$Distribution)
print("Number of successful surveys by distribution and block")
table(df_data$Distribution, df_data$Assigned.Block)
...
print("df_data represents full set of valid complete participants")
...
`r t_participants` individuals from Tumblr participated in Study 2, ( `r round(desc.gender.t[["Female"]],2)*100`% Female, `r round(desc.gender.t[["Male"]],2)*100`% Male, `r round(desc.gender.t[["Non-binary / third gender"]],2)*100`% Non-binary, `r (round((desc.gender.t[["Prefer not to say"]] + desc.gender.t[["Prefer to self-describe"]]),2)) * 100`% Other).
...
for (s in stimuli){
  ...
  plots <- as.list(lapply(colnames(cols), plot_sd, data = df, type ="S", split = TRUE, boxplot=TRUE))
