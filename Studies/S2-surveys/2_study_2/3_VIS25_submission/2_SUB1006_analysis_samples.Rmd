...
table(df_b1_participants$Assigned.Block, df_b1_participants$Sample)

```

#### wip mockup big figure just block 1 
```{r better-aggregation-by-sample, message=FALSE, warning = FALSE}
...
# if(GRAPH_SAVE == TRUE) {
#     ggsave(plot = x, path="figs/level_samples", filename =paste0("ALL_by_sample_ridges.png"), units = c("in"), width = 10, height = 14,  bg='#ffffff'  )}
...
## TODO ADD RUGS
 ...
 ####### CONFIDENCE
 df <- df_b1_graphs %>% filter(Assigned.Block==1) %>% select(Assigned.Block, Sample, )
 d <- df %>%  
        mutate(
          STUDY = fct_recode(Sample,
            Study_1  = "TUMBLR",
 ...
 d %>% ggplot(aes(x=STUDY, fill=MAKER_ID)) + 
...
## TUMBLR, GENERAL ONLY 
 ...
 df <- df_graphs %>% filter(Assigned.Block==1)
...
R <- df %>% 
  ggplot(aes(x=CONFIDENCE, y=Sample, fill = Sample, color = Sample)) + #fill=fct_rev(QUESTION))) + 
...
```{r aggregation-by-sample}



 df <- df_sd_questions_long_ALL %>% select(1:8, QUESTION, Sample, value)  
  d <- left_join( x = df, y = ref_labels, 
                  by = c("QUESTION" = "ref_sd_questions")) %>% 
        mutate(
          category=factor(category, levels=c("COMPETENCY","MAKER","CHART")),
...
(x <-
    ...
    labs (title = "ALL DATA by Sample", y = "", caption = "(point is median)") +
    cowplot::draw_text(text = ref_sd_questions, x = 100, y= ref_sd_questions,size = 8, vjust=-2, position=position_nudge(y=-.20))  + ##raw
    # cowplot::draw_text(text = ref_sd_questions, x = -4, y= ref_sd_questions,size = 10, vjust=-2) + ##z-score
    theme_minimal() + easy_remove_legend() 
)
...
## WIP SAMPLE SPARKLINES TABLE

```{r sample_spark_table}
...
df_tumblr_num <- df %>% filter(Sample =="TUMBLR") %>% select(all_of(all_q))
df_pilot_num <- df %>% filter(Sample %in% c("DATACOLLAR","BLUECOLLAR")) %>% select(all_of(all_q))
df_general_num <- df %>% filter(Sample == "GENERAL") %>% select(all_of(all_q))
...
t <- tinytable::tt(tab, theme = "void") %>% 
  plot_tt(j=2, i= 1:7, fun=g, data = df_cat, height = 1.5) %>% #tumblr
  plot_tt(j=3, i= 1:7, fun=g, data = df_cat, height = 1.5) %>% #blue/data
  plot_tt(j=4, i= 1:7, fun=g, data = df_cat, height = 1.5) %>% #general
  plot_tt(j=2, i= 8:22, fun="density", data = df_tumblr, color="darkgrey") %>%  #TUMBLR
  plot_tt(j=3, i= 8:22, fun="density", data = df_pilot, color="darkgrey") %>%  #BLUE/DATA
  plot_tt(j=4, i= 8:22, fun="density", data = df_general, color="darkgrey") %>%  #GENERAL
