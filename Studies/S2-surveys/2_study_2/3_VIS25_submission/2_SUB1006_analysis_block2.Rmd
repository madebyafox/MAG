...
#  
  ...
  # GGDIST HALFEYE (raincloud doesn't work b/c long tails)
g <-   ggplot(df, aes(y = fct_rev(QUESTION), x = CONFIDENCE)) +
    stat_halfeye(scale=0.6, density="bounded", point_interval = "median_qi", normalize="xy", alpha = 0.5) +
    stat_summary(fun=median, geom="text", colour="black",  fontface = "bold",
                vjust=+2, hjust = 0.50, aes( label=round(..x.., digits=0)))+
    stat_summary(fun="median", geom="point", size=2.2, color="black", fill="black") +
    facet_grid(.~STIMULUS) + 
  labs (title = "BLOCK 2 CONFIDENCE BY STIMULUS & QUESTION", y = "") +
  theme_minimal() + easy_remove_legend()
...
if(GRAPH_SAVE == TRUE){
  ggsave(plot = g, path="figs/level_block2/distributions", filename =paste0("CONFIDENCE","_ggdist.png"), units = c("in"), width = 10, height = 14  )
  }
...
for (s in stimuli){
  ...
  df <- df_b2  %>% 
    filter(STIMULUS==s) %>% 
    select(STIMULUS, MAKER_CONF, AGE_CONF, GENDER_CONF, TOOL_CONF) %>% 
  pivot_longer(
    cols = c(MAKER_CONF, AGE_CONF, GENDER_CONF, TOOL_CONF),
    names_to = "QUESTION",
    values_to = "CONFIDENCE"
  ...
  mutate(
    ...
    group_by(QUESTION) %>% 
    mutate(m=median(CONFIDENCE)) ## calc median for printing on graph


  # GGDIST HALFEYE (raincloud doesn't work b/c long tails)
  (g <- df %>%
      ggplot(aes(y = fct_rev(QUESTION), x = CONFIDENCE )) +
  ...
  if(GRAPH_SAVE == TRUE){ 
  ggsave(plot = g, path="figs/level_block2/distributions", filename =paste0("CONFIDENCE_",s,"_ggdist.png"), units = c("in"), width = 10, height = 14  )
  }
