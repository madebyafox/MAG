...
df_data <- readRDS("data/output/df_data.rds") %>% filter(Sample !="GENERAL") %>% filter(Assigned.Block ==1) %>% droplevels()
...
df_graphs <- readRDS("data/output/df_graphs.rds") %>% filter(Sample !="GENERAL") %>% filter(Assigned.Block ==1) %>% droplevels()
...
if(GRAPH_SAVE){ggsave(plot = (B+R), path="figs/level_samples/distributions", filename =paste0("CONFIDENCE_by_sample.png"), units = c("in"), width = 14, height = 8 ,  bg='#ffffff'  )}
...
if(GRAPH_SAVE){ggsave(plot = p, path="figs/level_samples/categoricals", filename =paste0("MAKER_by_sample.png"), units = c("in"), width = 14, height = 8 ,  bg='#ffffff'  )}
